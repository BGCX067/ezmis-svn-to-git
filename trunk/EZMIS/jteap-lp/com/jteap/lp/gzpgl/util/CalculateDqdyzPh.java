 ...
 *
 */
public class CalculateDqdyzPh implements CalculateFormula {
	...
	public String calculate(EFormExpContext context) {
		...
		try {
			...
			String sql = "select PH from TB_LP_GZP_DQDYZ where id = '" + docid + "'";
			...
			sql = "select max(PH) from TB_LP_GZP_DQDYZ where PH like '电气一" + date + "%'";
			...
			while (rs.next()) {
				...
				if (StringUtil.isNotEmpty(dqdyzNow)) {
					...
					retValue = "电气一" + date + "-" + dqdyzbh;
					return retValue;
				}
			}
			retValue = "电气一" + date + "-0001";
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (conn != null) {
