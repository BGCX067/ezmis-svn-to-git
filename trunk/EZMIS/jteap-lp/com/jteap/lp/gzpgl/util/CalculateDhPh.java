 ...
 *
 */
public class CalculateDhPh implements CalculateFormula {
	...
	public String calculate(EFormExpContext context) {
		...
		try {
			...
			String sql = "select PH from TB_LP_GZP_DH where id = '" + docid + "'";
			...
			sql = "select max(PH) from TB_LP_GZP_DH where PH like '动火" + date + "%'";
			...
			while (rs.next()) {
				...
				if (StringUtil.isNotEmpty(dqdyzNow)) {
					...
					retValue = "动火" + date + "-" + dqdyzbh;
					return retValue;
				}
			}
			retValue = "动火" + date + "-0001";
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (conn != null) {
