 ...
 *
 */
public class CalculateRljxPh implements CalculateFormula {
	...
	public String calculate(EFormExpContext context) {
		...
		try {
			...
			String sql = "select PH from TB_LP_GZP_RLJX where id = '" + docid + "'";
			...
			sql = "select max(PH) from TB_LP_GZP_RLJX where PH like '热机" + date + "%'";
			...
			while (rs.next()) {
				...
				if (StringUtil.isNotEmpty(dqdyzNow)) {
					...
					retValue = "热机" + date + "-" + dqdyzbh;
					return retValue;
				}
			}
			retValue = "热机" + date + "-0001";
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (conn != null) {
