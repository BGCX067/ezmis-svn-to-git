...
public class DqdyzNodeInitEventHandler extends InitNodeEventHandler {
		...
		try {
			...
			if ("开票".equals(taskNode.getName())) {
				...
				context.getContextInstance().setVariable("STATUS", status);
					...
					|| "安措签字".equals(taskNode.getName())
					|| "工作票批准".equals(taskNode.getName())) {
				status = "待许可";
				context.getContextInstance().setVariable("STATUS", status);
			} else if ("许可开工".equals(taskNode.getName())) {
				status = "待许可";
				context.getContextInstance().setVariable("STATUS", status);
			...
			String sql = "update TB_LP_GZP_DQDYZ set status='"+status+"' where id='"+docid+"'";
	...
	private void clearYQ(Connection conn, String docid) throws SQLException {
		String sql = "update TB_LP_GZP_DQDYZ set YQZT='', BGZT='' where id='"+docid+"'";
		Statement st = conn.createStatement();
		st.executeUpdate(sql);
		st.close();
	}
