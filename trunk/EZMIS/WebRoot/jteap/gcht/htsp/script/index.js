...
function initToolbarButtonStatus(button) {
	if (button.id == 'btnShenPiHt' || button.id == 'btnCheXiaoShenPi' || button.id == 'btnZuoFeiHt' || button.id == 'btnShanChuHt' || button.id=='btnXjzHt')
		button.setDisabled(true);
}

//新增补充合同
function btnXjzHt_Click() {
	...
	var htbh=select.json.HTBH;
	if(htbh.indexOf("补")!=-1){
		alert("补充合同不能新增补充合同!");
		return;
	}
	var url = link8 + "?flowConfigId=" + flowConfigId + "&param=" + id;
	showIFModule(url,"新增补充合同","true",735,582,{});
	rightGrid.getStore().reload();
}

//起草合同
function btnQicaoHt_Click() {
	var url = link8 + "?flowConfigId=" + flowConfigId + "&isZht=false";
	showIFModule(url,"起草合同","true",735,582,{});
	rightGrid.getStore().reload();
}
...
function btnZuoFeiHt_Click() {
	var select = rightGrid.getSelectionModel().getSelected();
	if (select.get('STATUS') != '起草合同') {
		alert('此合同不符合作废标准，无法作废');
		return;
	}
...
function btnShanChuHt_Click() {
	...
	if (window.confirm("确认删除选中的合同草稿吗?")) {
		var myMask = showExtMask("合同草稿删除中，请稍候。。。");
		// 提交数据
		Ext.Ajax.request({
			url : url,
			success : function(ajax) {
...
var searchAllFs = "合同编号#HTBH#textField,合同名称#HTMC#textField,合同类型#HTLX#textField,申请部门#SQBM#comboBox,创建年份#HTCJSJ#yearField".split(",");
var searchDefaultFs = "合同编号#HTBH#textField,合同名称#HTMC#textField,合同类型#HTLX#textField,申请部门#SQBM#comboBox,创建年份#HTCJSJ#yearField".split(",");
if(tableName == "TB_HT_GCHT"){
	searchAllFs = "合同编号#HTBH#textField,合同名称#HTMC#textField,合同类型#HTLX#textField,工程名称#GCMC#textField,申请部门#SQBM#comboBox,创建年份#HTCJSJ#yearField".split(",");
	searchDefaultFs = "合同编号#HTBH#textField,合同名称#HTMC#textField,合同类型#HTLX#textField,工程名称#GCMC#textField,申请部门#SQBM#comboBox,创建年份#HTCJSJ#yearField".split(",");
}
