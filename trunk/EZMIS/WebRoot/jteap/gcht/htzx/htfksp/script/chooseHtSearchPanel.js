...
SearchPanel = function(config) {
	...
	this.searchClick = function() {
		...
		var queryParamsSql = "?tableName="+tableName;
		Ext.each(oItems, function(oItem) {
					if (oItem.hidden == false) {
						...
						if (tempValue != null && tempValue != "") {
							if (temp.id.split("#")[1] == "htbh") {
								queryParamsSql += "&htbh="+encodeURIComponent(tempValue);
							}else if (temp.id.split("#")[1] == "htmc") {
								queryParamsSql += "&htmc="+encodeURIComponent(tempValue);
							}else if (temp.id.split("#")[1] == "htlx") {
								queryParamsSql += "&htlx="+encodeURIComponent(tempValue);
	...
	this.clearSearch = function() {
		...
		rightGrid.changeToListDS(link18+"?tableName="+tableName);
		rightGrid.getStore().reload();
	};
