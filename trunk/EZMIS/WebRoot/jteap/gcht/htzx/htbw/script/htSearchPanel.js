...
SearchPanel = function(config) {
	...
	this.searchClick = function() {
						...
						if (tempValue != null && tempValue != "") {
							if(temp.triggerClass=="x-form-date-trigger"){
				 				...
				 				if(temp.id.split("#")[1]=="beginYmd"){
				 					queryParamsSql+=" to_char(a.htcjsj,'yyyy-MM-dd')>='"+tempValue + "' and ";
		...
		var url = link6+"?queryParamsSql="+queryParamsSql.substring(0,queryParamsSql.length-5)+"&tableName="+tableName;
...
Ext.extend(SearchPanel, Ext.app.SearchPanel, {
	...
	appendSearchField: function(searchField,bHide){
		...
		tmpPanel=new Ext.Panel({labelWidth:100,layout:'form',columnWidth:.5,id:'SFP#'+sId,
			items:[oSearchField]
		});
