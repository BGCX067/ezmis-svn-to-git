...
<html>
	<head>
		...
		<title>JTEAP 2.0 文档附件在线编辑</title>
		<link rel="stylesheet" href="index.css" type="text/css"></link>
		<script>
			var docId = "${param.docId}" ; 		//文档ID
			var attachId="${param.attachId}";	//文档附件ID 
		</script>
		...
		<script type="text/javascript" src="script/DataSetTree.js"></script>
		<script type="text/javascript" src="script/DataSetGrid.js"></script>
		...
		<script type="text/javascript" src="script/ntkoocx.js"></SCRIPT> 
	</head>
	<body onload="onload();" onresize="windowResize();">
		<div id="bgDiv">
			<FORM id="myForm" METHOD="POST" ENCTYPE="multipart/form-data" ACTION="${contextPath}/jteap/doclib/DoclibAttachAction!modifyAttachOnLine.do" >	
				<div id="toolBar" style="margin:2px 2px 2px 2px;height:30px;padding:10 10 10 10;background-color: #efefef;border: solid 1px #e1e1e1">
					<input type="hidden" name="attachId" value="${param.attachId}"/>
					<button class="op" onClick="save();">保&nbsp;&nbsp;存</button>&nbsp;
					<button class="op" onClick="saveLocalFile();">保存本地文件</button>&nbsp;
					<button name="btnFull" onClick="switchFullScreen();">全&nbsp;&nbsp;屏</button>&nbsp;
					<button onClick="window.close();">退&nbsp;&nbsp;出</button>
				</div>
				<div id="splash">
					<img src="icon/loading.gif">
				</div>
				<div id="dataset" style="float: left;"></div>		
