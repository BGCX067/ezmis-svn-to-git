...
var DoclibCatalogAddWindow=function(resourceId){
	var formWindow=this;
	...
	//文档分类代码
	var txtDoclibCatalogCode = new Ext.form.TextField({fieldLabel: '分类代码',id:'txtDoclibCatalogCode',name:'catalogCode',maxLength:500,minLength:1,anchor:'90%'});
	...
	//文档排序序号
	var xhDoclibCatalogCode = new Ext.form.TextField({fieldLabel: '序号',id:'xhDoclibCatalogCode',name:'sortNo',value:time,maxLength:500,minLength:1,anchor:'90%'});
	...
	buttons: [{
			...
			handler:function(){
				...
				var params={};
				params.id=$('id').value;
				params.ops=$('ops').value;
				params.levelID=$('levelID').value;
				params.parentId=$('parentId').value;
				params.cmbDoclibLevel=$('cmbDoclibLevel').value;
				params.cmbDoclibTemplateFile=$('cmbDoclibTemplateFile').value;
				params.title=$('title').value;
				params.template=$('template').value;
				params.catalogCode=$('catalogCode').value;
				params.sortNo=$('sortNo').value;
	///////////////////////////////////////////////////////////////////////////////////////////////////			
	...
	var checkUnique = function(){
		var unique = false ;
		var catalogCode = simpleForm.findById('txtDoclibCatalogCode').getValue();
		var myAjax = new Ajax.Request(link14,{
			method: 'post', 
			parameters:"catalogCode="+catalogCode,
	    	asynchronous:false,//同步调用
	    	onComplete: function(req){
	    		var responseText=req.responseText;
	 			var responseObject = responseText.evalJSON();
	 			var data=responseObject;
	 			if(data.flag) unique = true ;
	 		},
	    	onFailure: function(e){
	    		alert("请求失败："+e);
	    	}
	...
	DoclibCatalogAddWindow.superclass.constructor.call(this,{
        title: '文档分类信息添加',
        width: 600,
        height:500,
        modal:true,
...
Ext.extend(DoclibCatalogAddWindow, Ext.Window, {
	/**
	 * 加载已经配置了模块信息的xml配置文件
	 */
