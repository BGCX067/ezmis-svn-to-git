...
var EditFormWindow=function(){
		...
		var groupDefaultId="";
		if(Node&&!Node.dissociation&&!Node.isRootNode()){
			groupDefaultId=Node.id;	
		}
		var formWindow=this;
		//把XML中的文件解析成人员的属性
		Ext.Ajax.request({
			...
			var tree=new Ext.tree.TreePanel({
		    	...
		    	root:new Ext.app.CheckboxAsyncTreeNode({
		        	...
		        	loader:new Ext.app.CheckboxTreeNodeLoader({
			            dataUrl:link2,listeners:{load:function(){
		            	//将树加载完之后将已经指定的资源勾选上
	            		var defaultId=groupDefaultId;
	...
	this.load=function load(){
		...
		// 姓名
		var txtUserName={xtype:'textfield',fieldLabel:'姓名',name:'userName',maxLength:10,minLength:2,regex:/^\S+$/,regexText:'姓名中不能包含空格',inputType:'textfield',allowBlank:false,anchor:'90%'};
		
		var infoUserName=new Ext.app.LabelPanel('用户真实姓名，2-10汉字或有效字符');
		
		
		//工号
		var txtUserLoginName={xtype:'uniquetextfield',notUniqueText:'该工号已经存在，请使用其他工号',url:link28,id:'txtUserLoginName',fieldLabel:'工号',vtype:'alphanum',maxLength:4,minLength:4,minLengthText:'只能输入4位字母数字组合',maxLengthText:'只能输入4位字母数字组合',name:'userLoginName',allowBlank:false,anchor:'90%',
