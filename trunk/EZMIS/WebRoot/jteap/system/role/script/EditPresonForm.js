...
var EditPresonFormWindow=function(personid){
	...
	//姓名
	var txtUserName={xtype:'textfield',id:'username',fieldLabel:'中文名',name:'userName',regex:/^\S+$/,regexText:'姓名中不能包含空格',maxLength:10,minLength:2,inputType:'textfield',allowBlank:false,anchor:'90%'};
	var infoUserName=new Ext.app.LabelPanel('用户真实姓名，2-10汉字或有效字符');

	//工号
	var txtUserLoginName={xtype:'uniquetextfield',id:'txtUserLoginName',notUniqueText:'该工号已经存在，请使用其他工号',url:link30+"?personId="+personid,vtype:'alphanum',fieldLabel:'工号',maxLength:4,minLength:4,name:'userLoginName',allowBlank:false,anchor:'90%'
						,listeners:{blur:function(txtField){
												}}};
	var infoUserLoginName=new Ext.app.LabelPanel('用户工号，4位字母及数字');
	
	//登录名
	var txtUserLoginName2={xtype:'uniquetextfield',id:'txtUserLoginName2',fieldLabel:'登录名',url:link32+"?personId="+personid,maxLength:12,minLength:2,name:'userLoginName2',allowBlank:false,anchor:'90%'
						,listeners:{blur:function(txtField){
												}}};
	...
	buttons: [{
			...
			handler:function(){
				...
				if(groupTree.rendered){
					groupIds=groupTree.getRootNode().getCheckedIds(true,false);
					bGroupSetup=true;
				}
