...
Ext.extend(RightGrid, Ext.grid.GridPanel, {
			...
			getDefaultDS : function(status) {
				...
				if (status == 'dcl') {
					url = link1;
					procFields = ["ID", "SBMC", "STATUS", "SQBM", "SQR", "SQSJ", "FLOW_TOPIC", "TASKTODOID", "FLOW_NAME",
							"FLOW_INSTANCE_ID", "CURRENT_TASKNAME", "POST_PERSON", "POST_TIME", "TOKEN", "CURSIGNIN",
							"curSignInName"];
					// 已处理
				} else if (status == 'ycl') {
					url = link2;
					procFields = ["ID", "SBMC", "STATUS", "SQBM", "SQR", "SQSJ",  'ID_', 'FLOW_NAME', 'START_', 'END_',
					...
					cm = new Ext.grid.ColumnModel([this.sm, {
							...
							/*	
							{
								...
								renderer:function(value,metadata,record,rowIndex,colIndex,store){
									var sqsj = formatDate(new Date(value), "yyyy-MM-dd hh:mm:ss");
									return sqsj;
								}
							}
							*/
