...
RightGrid = function() {
	...
	this.on("celldblclick", function(grid, rowIndex, columnIndex, e) {
				...
				if (node.id == 'dcl') {
					...
					// 弹出流程查看窗口
					var url = link9;
					var windowUrl = link10 + "?pid=" + pid + "&id=" + id
							+ "&token=" + token + "&isEdit=true";
					...
					var url = link9;
					var windowUrl = link10 + "?pid=" + select.get("ID_")
							+ "&status=false";
					var args = "url|" + windowUrl + ";title|" + '查看流程';
					var retValue = showModule(url, "yes", 800, 600, args);
				}
...
Ext.extend(RightGrid, Ext.grid.GridPanel, {
			...
			getDefaultDS : function(status) {
				...
				if (status == 'dcl') {
					url = link1;
					procFields = ["ID", "SBMC", "STATUS", "SQBM", "SQR",
							"SQSJ", "FLOW_TOPIC", "TASKTODOID", "FLOW_NAME",
					...
					url = link2;
					procFields = ["ID", "SBMC", "STATUS", "SQBM", "SQR",
							"SQSJ", 'ID_', 'FLOW_NAME', 'START_', 'END_',
			...
			getColumnModel : function(status) {
				...
				if (status == 'dcl') {
					...
					/*
					 * { id : 'SQBM', header : "申请部门", width : 100, sortable :
					 * true, dataIndex : 'SQBM' }, { id : 'SQR', header : "申请人",
					 * width : 80, sortable : true, dataIndex : 'SQR' }, { id :
					 * 'SQSJ', header : "申请时间", width : 140, sortable : true,
					 * dataIndex : 'SQSJ',
					 * renderer:function(value,metadata,record,rowIndex,colIndex,store){
					 * var sqsj = formatDate(new Date(value), "yyyy-MM-dd
					 * hh:mm:ss"); return sqsj; } }
					 */
					]);
					// 其他
				} else {
					cm = new Ext.grid.ColumnModel([this.sm, {
							...
							/*
							 * { id : 'SQR', header : "申请人", width : 80,
							 * sortable : true, dataIndex : 'SQR' }, { id :
							 * 'SQSJ', header : "申请时间", width : 140, sortable :
							 * true, dataIndex : 'SQSJ',
							 * renderer:function(value,metadata,record,rowIndex,colIndex,store){
							 * var sqsj = formatDate(new Date(value),
							 * "yyyy-MM-dd hh:mm:ss"); return sqsj; } },
							 */
							{
								id : 'START_',
								header : "开始时间",
								...
								renderer : function(value, metadata, record,
									...
									var dt = formatDate(new Date(value),
											"yyyy-MM-dd hh:mm:ss");
									return dt;
								}
							}, {
