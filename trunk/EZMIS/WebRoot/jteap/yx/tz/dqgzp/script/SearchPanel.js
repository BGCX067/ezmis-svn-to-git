...
SearchPanel = function(config) {
	...
	this.searchClick = function() {
		...
		Ext.each(oItems, function(oItem) {
			if (oItem.hidden == false) {
				...
				if (temp.triggerClass == "x-form-date-trigger") {
					if (temp.id.split("#")[1] == "pzgzqx1") {
						tempValue = formatDate(tempValue, "yyyy-MM-dd");
						queryParamsSql += "to_char(obj.pzgzqx,'yyyy-mm-dd') >= '" + tempValue + "' and ";
					} else if (temp.id.split("#")[1] == "pzgzqx2") {
						tempValue = formatDate(tempValue, "yyyy-MM-dd");
						queryParamsSql += "to_char(obj.pzgzqx,'yyyy-mm-dd') <= '" + tempValue + "' and ";
					} else if (temp.id.split("#")[1] == "xksj1") {
						tempValue = formatDate(tempValue, "yyyy-MM-dd");
						queryParamsSql += "to_char(obj.xksj,'yyyy-mm-dd') >= '" + tempValue + "' and ";
					} else if (temp.id.split("#")[1] == "xksj2") {
						tempValue = formatDate(tempValue, "yyyy-MM-dd");
						queryParamsSql += "to_char(obj.xksj,'yyyy-mm-dd') <= '" + tempValue + "' and ";
					} else if (temp.id.split("#")[1] == "zjsj1") {
						tempValue = formatDate(tempValue, "yyyy-MM-dd");
						queryParamsSql += "to_char(obj.zjsj,'yyyy-mm-dd') >= '" + tempValue + "' and ";
					} else if (temp.id.split("#")[1] == "zjsj2") {
						tempValue = formatDate(tempValue, "yyyy-MM-dd");
						queryParamsSql += "to_char(obj.zjsj,'yyyy-mm-dd') <= '" + tempValue + "' and ";
					}
					// 其他字段模糊查询
		...
		var url = link1 + "?queryParamsSql=" + queryParamsSql.substring(0, queryParamsSql.length - 5) + "&type=" +type;
...
Ext.extend(SearchPanel, Ext.app.SearchPanel, {
	appendSearchField : function(searchField, bHide) {
		...
		if (sType == "dateFieldShowTime") {
			...
			var storeZt = new Ext.data.Store({
				data : {
					rows : distsZt
				},
