...
function btnAdd_Click(){
	/*var url = CONTEXT_PATH+"/jteap/form/eform/eformRec.jsp?formSn="+formSn+"&nowYmd="+nowYmd+"&nowBc="+encodeURIComponent(nowBc);
	var mylink = link2+"?formSn="+formSn+"&nowYmd="+nowYmd+"&nowBc="+encodeURIComponent(nowBc);
	...
	var url = CONTEXT_PATH+"/jteap/form/eform/eformRec.jsp?formSn="+formSn+"&nowYmd="+new Date().format("Y-m-d H:i")+"&nowBc="+encodeURIComponent(nowBc);
	var myTitle = "添加记录";
	var fw = new $FW({
		url:url,
		width:750,
...
function btnModi_Click(){
	...
	var url = CONTEXT_PATH+"/jteap/form/eform/eformRec.jsp?formSn="+formSn+"&nowYmd="+new Date().format("Y-m-d H:i")+
				"&nowBc="+encodeURIComponent(select.data.ZBBC)+"&docid="+id;
	var myTitle = "修改记录";
...
function btnStop_Click(){
 	var dataurl = "";
 	if(window.confirm("确认进行停机处理？")){
 		var url = contextPath+"/jteap/yx/runlog/RltjAction!getOldId.do?formSn="+formSn;
		var conn = Ext.lib.Ajax.getConnectionObject().conn; 
		...
		if(responseObject.success == true){
	 		var url = CONTEXT_PATH+"/jteap/form/eform/eformRec.jsp?formSn="+formSn+"&nowYmd="+new Date().format("Y-m-d H:i")+
					"&docid="+responseObject.id+"&op=New";
			var myTitle = "停机处理";
			var fw = new $FW({
				url:url,
				width:750,
			...
			fw.show();
	 		...
	 		alert("请确认"+responseObject.msg+"有填写记录！");
	 	}	
 	}
}
//删除
function btnDel_Click(){
	...
	if(window.confirm("确认删除选中的燃炉统计数据吗？")){
		Ext.Ajax.request({
			url:url,
...
var searchAllFs="开始时间#beginYmd#dateField,结束时间#endYmd#dateField,值班班次#nowBc#comboBox,值班值别#zbzb#comboBox,抄表人#cbr#textField".split(",");
//查询面板中默认显示的条件，格式同上
var searchDefaultFs="开始时间#beginYmd#dateField,结束时间#endYmd#dateField,值班班次#nowBc#comboBox,值班值别#zbzb#comboBox,抄表人#cbr#textField".split(",");
var searchPanel=new SearchPanel({searchDefaultFs:searchDefaultFs,searchAllFs:searchAllFs});

//设置查询条件为当前时间、班次
Ext.getCmp("sf#beginYmd").setValue(nowYmd);
