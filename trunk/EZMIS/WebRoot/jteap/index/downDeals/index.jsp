		...
		<script type="text/javascript">
			...
			function alertWindow(alertWindowUrl,id,tr_index){
				...
				if(alertWindowUrl != "null" && alertWindowUrl != "" && id != "null" && id != ""){
					...
					showIFModule(alertWindowUrl,"已办事项","true",317,400,{});
					window.location.reload();
					window.parent.parent.frames["dbybIF"].frames["dealsIF"].location.reload();
				}	
			}
			
		</script>
		
		<div style="background-color: #FFF;width: 99.5%;">
			<div>
				...
				<div class="Aidemenu">
					...
					<%
						...
						Map<String, List<TaskToDo>> map = taskToDoManager.findCurrentLoginDeals(curPersonLoginName,false);
						if(map.size() < 1){
							out.print("<font style='color:green;font-size:12px;font-weight:bold;'>暂无已办事项...</font>");
						}
						...
						for(Map.Entry<String, List<TaskToDo>> entry : map.entrySet()){
							...
							int j=0;
							while(j<list.size()){
							String token = list.get(j).getFlowTopic();
						...
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="AidTAB">
							...
							<tr id="tr_<%=i%>" onclick='alertWindow("<%=taskToDo.getAlertWindowUrl()%>","<%=taskToDo.getDocId()%>","<%=i%>")' >
								...
								<td width="15%" class="<%=tdCls_2%>">
									<%=dateFormat.format(taskToDo.getDealTime()) %>
								</td>
							</tr>
							<%
							}
