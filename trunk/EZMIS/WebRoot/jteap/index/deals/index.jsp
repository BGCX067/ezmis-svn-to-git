				...
				<div class="Aidemenu">
					...
					<%
						...
						for(Map.Entry<String, List<TaskToDo>> entry : map.entrySet()){
						    ...
						    if(("运行管理").equals(entry.getKey())){
						    	continue;
						    }else if(("物资需求计划申请").equals(entry.getKey())){
						    	for(int i=0; i<list.size(); i++){
									TaskToDo taskToDo = list.get(i);
									if(taskToDo.getCurSignIn() != null){
										Person person = personManager.get(taskToDo.getCurSignIn().getId());
										if(("物资需求计划申请").equals(taskToDo.getChildSystemName()) && !curPersonLoginName.equals(person.getUserLoginName())){
											list.remove(taskToDo);
										}
									}
								}
						    }
					...
					<a class="menuitem submenuheader" href="#"><%=entry.getKey() %>（<%=list.size() %> 条记录）</a>
						...
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="AidTAB">
							...
							for(int i=0; i<list.size(); i++){
								...
								if(("运行管理").equals(taskToDo.getChildSystemName())){
									continue;
								}
								//tr 样式控制
								String tdCls_1 = "";
