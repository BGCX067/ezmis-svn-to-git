...
LeftTree = function() {
	...
	treeEditor.on("beforecomplete", function(editor, value, oldValue) {
		...
		// 如果不是新创建的节点,新创建的节点的id是以ynode打头的，保存成功后会更新其id属性
			...
			Ext.Ajax.request({
				...
				params : {
					nodeName : value,
					id : editor.editNode.id.indexOf("ynode") >= 0 ? "" : editor.editNode.id,
					preSortNo : preSortNo
				}
			})
			bFlag = false;
	...
	this.getSelectionModel().on("selectionchange", function(oSM, oNode) {
		// 修改菜单栏按钮状态
			var btnAddAqcsCatalog = mainToolbar.items.get("btnAddAqcsCatalog");
			var btnModiAqcsCatalog = mainToolbar.items.get("btnModiAqcsCatalog");
			var btnDelAqcsCatalog = mainToolbar.items.get("btnDelAqcsCatalog");
			var btnAddAqcs = mainToolbar.items.get("btnAddAqcs");

			if (oNode == null) {
				if (btnAddAqcsCatalog)
					btnAddAqcsCatalog.setDisabled(true);
				if (btnModiAqcsCatalog)
					btnModiAqcsCatalog.setDisabled(true);
				if (btnDelAqcsCatalog)
					btnDelAqcsCatalog.setDisabled(true);
				if (btnAddAqcs)
					btnAddAqcs.setDisabled(true);
				this.tree.root.select();
			} else {
				if (btnAddAqcsCatalog)
					btnAddAqcsCatalog.setDisabled(false);
				...
				if (oNode.isRootNode()) {
					...
					if (btnModiAqcsCatalog)
						btnModiAqcsCatalog.setDisabled(false);
					if (btnDelAqcsCatalog)
						btnDelAqcsCatalog.setDisabled(false);
					if (btnAddAqcs)
						btnAddAqcs.setDisabled(false);
				}

				var oNodeId = oNode.isRootNode() ? "" : oNode.id
				var url = link4 + (oNode.isRootNode() ? "" : "?catalogId=" + oNodeId);
