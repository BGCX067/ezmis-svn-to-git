...
function btnCancel_Click() {
	...
	if (window.confirm('是否作废此申请单？')) {
		...
		if (dt != null) {
			url = contextPath + "/jteap/lp/gzpgl/P2RoleExtendForLpAction!validatePersonInRole.do";
			Ext.Ajax.request({
				url : url,
				method : 'POST',
				params : {
...
function btnDelSq_Click() {
	...
	if (window.confirm("确认删除选中的工作票草稿吗?")) {
		var myMask = showExtMask("工作票草稿删除中，请稍候。。。");
		// 提交数据
		Ext.Ajax.request({
			url : url,
			success : function(ajax) {
