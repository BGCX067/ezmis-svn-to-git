...
Ext.extend(RightGrid, Ext.grid.GridPanel, {
	...
	getDefaultDS : function(status, params) {
		...
		if (status == 'dcl') {
			url = link1;
			procFields = ["PH", "BZMC", "JHKSSJ", "JHJSSJ", "FZRMC", "STATUS", "ID", "FLOW_TOPIC", "TASKTODOID",
					"FLOW_NAME", "FLOW_INSTANCE_ID", "CURRENT_TASKNAME", "POST_PERSON", "POST_TIME", "TOKEN",
					"CURSIGNIN", "curSignInName", "TABLENAME"];
			// 已处理
	} else if (status == 'ycl') {
		url = link2;
		procFields = ["PH", "BZMC", "JHKSSJ", "JHJSSJ", "FZRMC", "STATUS", "ID", 'ID_', 'FLOW_NAME', 'START_', 'END_',
				'PROCESSINSTANCE_', 'FLOW_CONFIG_ID', "TABLENAME"];
		// 草稿箱
	} else if (status == 'cgx') {
		url = link3;
		procFields = ["PH", "BZMC", "JHKSSJ", "JHJSSJ", "FZRMC", "STATUS", "ID", 'ID_', 'FLOW_NAME', 'START_', 'END_',
