...
SearchPanel = function(config) {
	...
	this.searchClick=function(){
		...
		var flag = false;
	 		...
	 		if(oItem.hidden==false){
	 			...
	 			if(tempValue!=null&&tempValue!=""){
		 			...
		 			if(temp.triggerClass=="x-form-date-trigger"){
						var nf = tempValue.getFullYear();
						var yf = tempValue.getUTCMonth()+1;
		 				if(yf<10){
 		...
 		rightGrid.reconfigure(getStore(url),getColumns());
	};  
	// 清空查询条件
...
//	 	var txtWidth=30;
		...
		//日期类型
			...
			//评级级别数据源
			var url=contextPath + "/jteap/gcht/htsp/HeTongCatalogAction!getHtCatalogAction.do";
			var storeLevel = new Ext.data.JsonStore({
				url : url,
				root:'data',
				fields:['id','catalogName']
			})
			//评级级别
			if(arrays[1] == 'gcxm'){
