			...
			if($('dates').value!=""){
				...
				Ext.Ajax.request({
					...
					failure:function(){
				 	...
				 	method:'POST',
				 	params: {nf:nf,yf:yf,rwlb:'fytj'}//Ext.util.JSON.encode(selections.keys)		
				});
			}else{
				alert("请选择年月！");
				return;
...
exportExcels = function(obj, flag,nf,yf) {
	...
	path = contextPath +"/jteap/wz/fytj/FytjAction!exportExcel.do?nf="+nf+"&yf="+yf;
		...
		if(e=='yes'){
			...
			Ext.Ajax.request({
				...
				failure:function(){
			 	...
			 	method:'POST',
			 	params: {nf:nf,yf:yf,lb:'fytj'}//Ext.util.JSON.encode(selections.keys)		
			});
		}
	});
}
