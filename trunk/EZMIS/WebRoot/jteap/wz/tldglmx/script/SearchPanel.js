	 		...
	 		if(oItem.hidden==false){
	 			...
	 			if(tempValue!=null&&tempValue!=""){
		 			...
		 			if(temp.triggerClass=="x-form-date-trigger"){
		 				...
		 				}else if(temp.id.split("#")[1]=="tlStart"){
		 					sxStart = tempValue;
		 					queryParamsSql+="to_char(obj.tldgl.tlsj,'yyyy-mm-dd hh24:mi')>='"+tempValue+"' and ";
		 				}else if(temp.id.split("#")[1]=="tlEnd"){
		 					queryParamsSql+="to_char(obj.tldgl.tlsj,'yyyy-mm-dd hh24:mi')<='"+tempValue+"' and ";
		 					if(sxStart!=null){
...
//         		 editable:true,selectOnFocus:true,
			...
			oSearchField = new Ext.form.ComboBox({  
				...
				store:new Ext.data.Store({
					baseParams:{PAGE_FLAG:'PAGE_FLAG_NO'},
					autoLoad : true, 
					proxy: new Ext.data.ScriptTagProxy({url : link7}),
					reader: new Ext.data.JsonReader({root: 'list'},
							['id','ckmc'])
				}),  
                 valueField:'id',  
