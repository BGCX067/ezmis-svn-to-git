
/**
 * 字段列表
 */
RkGrid=function(){
    var defaultDs=this.getDefaultDS(link4);
    var grid=this;
    this.pageToolbar=new Ext.PagingToolbar({
	    pageSize: CONSTANTS_PAGE_DEFAULT_LIMIT,
	    store: defaultDs,
	    displayInfo: true,
	    displayMsg: '共{2}条数据，目前为 {0} - {1} 条',
		emptyMsg: "没有符合条件的数据",
		items:['-',{text:'导出Excel',handler:function(){
		exportExcel(grid,true);
		}},'-','<font color="red">*双击查看详细信息</font>']
	});
	RkGrid.superclass.constructor.call(this,{
	 	ds: defaultDs,
	 	cm: this.getColumnModel(),
		sm: this.sm,
	    margins:'2px 2px 2px 2px',
		width:600,

		height:300,
		loadMask: true,
		maskDisabled :false,
		frame:true,
		region:'center',
		tbar:this.pageToolbar 
	});	
	
	//当有用户被选择的时候，显示工具栏的修改和删除按钮
	this.getSelectionModel().on("selectionchange",function(oCheckboxSModel){
		
		var btnDelCForm=mainToolbar.items.get('btnDelCForm');
		var btnModifyCForm=mainToolbar.items.get('btnModifyCForm');
		
		if(oCheckboxSModel.getSelections().length==1){
			if(btnModifyCForm) btnModifyCForm.setDisabled(false);
		}else{
			if(btnModifyCForm) btnModifyCForm.setDisabled(true);
		}
		
		if(oCheckboxSModel.getSelections().length<=0){
			if(btnDelCForm) btnDelCForm.setDisabled(true);
		}else{
			if(btnDelCForm) btnDelCForm.setDisabled(false);
		}
	});
	
	this.on("celldblclick",function(grid, rowIndex, columnIndex, e){
		var select=rkGrid.getSelectionModel().getSelections()[0];
		if(select.get("type")=="EFORM"){
			//var eformUrl=select.get("eformUrl");
			
			var url="/jteap/cform/fceform/common/djframe.htm?djsn="+select.get("sn")+"&catalogName="+select.get("eformUrl")
		    window.open(CONTEXT_PATH+url);
		}
		
		if(select.get('type')=="EXCEL"){
			var url=CONTEXT_PATH+"/jteap/cform/excelFormRec.jsp?cformId="+select.json.id;
			var features="menubar=no,toolbar=no,width=800,height=600";
			window.open(url,"_blank",features);
		}
		
		if(select){
			
			var url=link15+"&docid="+select.json.crkrzgl.id;
			var params = 'crklbmc|'+$dictKey("RKRZLB",select.json.crkrzgl.crklb)+';'+
						 'ckmc|'+select.json.crkrzgl.ck.ckmc;
			result=showModule(url,true,800,645,params);
		}
	});
	
}
Ext.extend(RkGrid, Ext.grid.GridPanel, {
	sm:new Ext.grid.RowSelectionModel({singleSelect:true}),
	
	
	/**
	 * 取得默认数据源
	 * 返回数据格式
	 * {success:true,totalCount:20,list:[{id:'123',field1:'111',...,field5'222'},{...},...]} 
	 */
	getDefaultDS:function(url){
	    var ds = new Ext.data.Store({
	        proxy: new Ext.data.ScriptTagProxy({
	            url: url
	        }),
	        reader: new Ext.data.JsonReader({
	            root: 'list',
	            totalProperty: 'totalCount',
	            id: 'id'
	        }, [
	        "sysl",
			"syje",
			"wzda",
			"wzda.dqkc",
			"wzda.jhdj",
			"wzda.yfpsl",
			"crksl",
			"id",
			"crkjg",
			"crkdh",
			"xh",
			"crkrzgl",
			"crkrzgl.crkdh",
			"crkrzgl.id",
			"crkrzgl.crksj",
			"crkrzgl.crklb",
			"crkrzgl.ck",
			"crkrzgl.ck.ckmc",
	        ]),
	        remoteSort: true
	    });
		return ds;
	},
    
	/**
	 * PersonAction 列模型
	 */
	getColumnModel:function(){
	    var cm=new Ext.grid.ColumnModel([
		    	//this.sm,
		    	{id:'xh',header: "序号", width: 40, sortable: true, 
					renderer:function(value,metadata,record,rowIndex,colIndex,store){
					 	return rowIndex+1;
					}
				},
		    	{id:'crkrzgl.crkdh',header: "入库单编号", width: 100, sortable: true, dataIndex: 'crkrzgl.crkdh'},
		    	{id:'crkrzgl.crksj',header: "入库时间", width: 100, sortable: true, dataIndex: 'crkrzgl.crksj',
		    	 renderer:function(value,metadata,record,rowIndex,colIndex,store){
						var dt = formatDate(new Date(value["time"]),"yyyy-MM-dd"); 
						return dt;         
					}
		    	},
		    	{id:'crkrzgl.crklb',header: "入库类别", width: 100, sortable: true, dataIndex: 'crkrzgl.crklb',
		    		renderer:function(value,metadata,record,rowIndex,colIndex,store){
						return $dictKey("RKRZLB",value);
					}
		    	},
		    	{id:'crkjg',header: "入库价格", width: 100, sortable: true,align:'right', dataIndex: 'crkjg',
		    		renderer:function(value,metadata,record,rowIndex,colIndex,store){
						if(value==null){
							value = 0;
						}
						return parseFloat(value).toFixed(2);
					}
		    	},
		    	{id:'sysl',header: "剩余数量", width: 100, sortable: true,align:'right',dataIndex: 'sysl',
		    		renderer:function(value,metadata,record,rowIndex,colIndex,store){
		    			/*
		    			var dqkc = record.get('wzda.dqkc');
		    			var yfpsl = record.get('wzda.yfpsl');
						value = dqkc - yfpsl;
						return parseFloat(value).toFixed(2);
						*/
		    			if(value==null){
							value = 0;
						}
						return parseFloat(value).toFixed(2);
					}
		    	},
		    	{id:'syje',header: "剩余金额", width: 100, sortable: true,align:'right', dataIndex: 'syje',
		    		renderer:function(value,metadata,record,rowIndex,colIndex,store){
		    			/*
						var dqkc = record.get('wzda.dqkc');
		    			var yfpsl = record.get('wzda.yfpsl');
		    			var jhdj = record.get('wzda.jhdj');
		    			value = (dqkc-yfpsl)*jhdj;
						return parseFloat(value).toFixed(2);
						*/
		    			if(value==null){
							value = 0;
						}
						return parseFloat(value).toFixed(2);
					}
		    	}
			]);
		return cm;
	},
	/**
	 * 切换数据源->LogAction!showList
	 */
	changeToListDS:function(url){
		var ds = this.getDefaultDS(url);	
		var cm=this.getColumnModel();
		this.pageToolbar.bind(ds);
		this.reconfigure(ds,cm);
	},
	/**
	 * 删除
	 */
	deleteSelect:function(select){
		var selections = this.getSelections();//获取被选中的行
		var rkGrid=this;
		var ids="";
		Ext.each(selections,function(selectedobj){
			ids+=selectedobj.id+",";//取得他们的id并组装
		});
		if(window.confirm("确认删除选中的条目吗？")){
			Ext.Ajax.request({
				url:link5,
				success:function(ajax){
			 		var responseText=ajax.responseText;	
			 		var responseObject=Ext.util.JSON.decode(responseText);
			 		if(responseObject.success){
			 			alert("删除成功");
			 			rkGrid.getStore().reload();
			 		}else{
			 			alert(responseObject.msg);
			 		}				
				},
			 	failure:function(){
			 		alert("提交失败");
			 	},
			 	method:'POST',
			 	params: {ids:ids}//Ext.util.JSON.encode(selections.keys)			
			});
		}
	}

});

