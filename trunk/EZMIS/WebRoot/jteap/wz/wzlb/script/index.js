...
function btnAddLBModule_Click(){
	...
	if(select){
		...
		//window.open(url);
		showIFModule(url,"添加物资类别","true",380,280,{},null,null,null,false,"no");
...
function btnModiLBModule_Click(){
	var select=leftTree.getSelectionModel().getSelectedNode();
	if(select){
		var url = contextPath + "/jteap/form/eform/eformRec.jsp?formSn="+formSn+"&docid="+select.id+"&flbbm="+(select.parentNode.id=="root"?"":select.parentNode.id);
		var c = showIFModule(url,"修改物资类别","true",380,280,{},null,null,null,false,"no");
		leftTree.getRootNode().reload();
	}
}
/***********删除类别*************/
function btnDelLBModule_Click(){
	var url = contextPath + "/jteap/wz/wzlb/WzlbAction!deleteNodeAction.do";
		...
		if(window.confirm("确认删除选中的表单数据吗？")){
			Ext.Ajax.request({
				...
				success:function(ajax){
			 	...
			 	method:'POST',
			 	//params: {"nodeId":select.id}//Ext.util.JSON.encode(selections.keys)
			 	params:{nodeId:select.id}
			});
		}
	}		
}
...
function btnAddDAModule_Click(){
	...
	showIFModule(url,"添加物资档案","true",680,500,obj,null,null,null,false,"no");
...
function btnModiDAModule_Click(){
	...
	//	alert(select.json.id);
		...
		showIFModule(url,"修改物资档案","true",680,500,{},null,null,null,false,"no");
	//	window.open(url);
		rightGrid.getStore().reload();
	}
}
...
function btnGGLB_Click(){
	...
	var url = contextPath + "/jteap/wz/wzlb/wzlbTree.jsp";
		var str = showIFModule(url,"物资类别","true",200,580,{},null,null,null,false,"yes");
		if(str==null){
//			alert("请选择一项物资的类别");
			return;
		}
...
var dqkcWin =  new Ext.Window({
      ...
      //  applyTo:'hello-win',
        ...
        buttons:[{
                    text:'确定',
                    handler: function(){
                    	if(dqkcForm.getForm().findField("dqkc_textfield").getValue() == "" && typeof(dqkcForm.getForm().findField("dqkc_textfield").getValue()) != "number"){
                    		alert("当前库存量不能为空!");
                    		return ;
                    	}else {
	                    	dqkcForm.getForm().submit({
						         success : function() {
							          Ext.Msg.alert("成功", "修改成功");
							          dqkcForm.getForm().findField("dqkc_textfield").setValue("");
							          dqkcWin.hide();
							          rightGrid.getStore().reload();
						         },
						         failure : function() {
