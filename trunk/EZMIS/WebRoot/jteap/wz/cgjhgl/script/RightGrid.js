 ...
 */
RightGrid=function(){
    var defaultDs=this.getDefaultDS(link4+"?queryParamsSql=obj.zt!='4'");
    var grid=this;
...
//		}},'-','<font color="red"></font>']
	...
	this.on("cellclick",function(grid, rowIndex, columnIndex, e){
		...
		var whereSql = "obj.cgjhgl.id ='" + jhid +"' and obj.zt!='4'";
		var selected = rightGrid.getSelectionModel().getSelected();
		if(!selected){
			whereSql = "obj.cgjhgl.id ='' and obj.zt!='4'";
		}
		mxGrid.store.baseParams={PAGE_FLAG:'PAGE_FLAG_NO',queryParamsSql:whereSql,zt:0};
		mxGrid.store.load();
	});
...
//				{id:'jhy',header: "计划人", width: 100, sortable: true, dataIndex: 'jhy'}
	...
	deleteSelect:function(select){
		var selections = mxGrid.getSelections();//获取被选中的行
		//var mxGrid=this;
		var ids="";
		var cgjhid = rightGrid.getSelectionModel().getSelected().get('id');
				...
				success:function(ajax){
			 		...
			 		if(responseObject.success){
			 			...
			 			rightGrid.getStore().on('load',function(){
							var whereSql = "obj.cgjhgl.id ='" + cgjhid +"' and obj.zt!='4'";
...
Ext.extend(MxRightGrid, Ext.grid.GridPanel, {
	...
	getDefaultDS:function(url){
	    ...
	    var ds = new Ext.data.Store({
			...
			'id','cgjhgl','cgjhgl.bh','cgjhgl.jhy','cgjhgl.zdsj','cgjhgl.sxsj',
			'cgjhgl.bz','wzdagl','wzdagl.wzmc','wzdagl.xhgg','wzdagl.kw','wzdagl.kw.cwmc',
			'wzdagl.kw.ck','wzdagl.kw.ck.id','wzdagl.kw.ck.ckmc','xh','jhdj','cgjldw','cgsl',
			'hsxs','jhdhrq','dhsl','cgfx','cgy','je','zt','person','person.id','person.userName',"XqjhDetail","dyszt"
	        ]),
	        remoteSort: true
	    });
        //onload事件
...
//				{id:'id',header: "ID", width: 100, sortable: true, dataIndex: 'id'},
				...
				{id:'wzbm',header: "物资名称", width: 100, sortable: true, dataIndex: 'wzdagl.wzmc'},
...
//				{id:'bz',header: "备注", width: 100, sortable: true, dataIndex: 'cgjhgl.bz'}
	...
	enableCgjh:function(select){
		...
		Ext.each(selections,function(selectedobj){
			...
			cglid = selectedobj.data.cgjhgl.id;
		});
		var doEnable = false;
		var ds = mxGrid.getDefaultDS(link2);
		var whereSql = "obj.id in (" + idsIn.substring(0,idsIn.length-1) +")";
						...
						Ext.Ajax.request({
							...
							success:function(ajax){
						 		...
						 		if(responseObject.success){
						 			alert("生效成功!");
						 			rightGrid.getStore().on('load',function(){
										var whereSql = "obj.cgjhgl.id ='"+cglid+"'";
										mxGrid.store.baseParams={PAGE_FLAG:'PAGE_FLAG_NO',queryParamsSql:whereSql};
						 	...
						 	method:'POST',
						 	params: {ids:ids,cglid:cglid}//Ext.util.JSON.encode(selections.keys)			
						});
					}
				}
			}else{
