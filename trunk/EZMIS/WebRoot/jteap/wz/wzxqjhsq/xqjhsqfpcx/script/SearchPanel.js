...
var SearchPanel = function(config){
	...
	this.searchClick=function(){
	 			...
	 			if(tempValue!=null&&tempValue!=""){
	 				if(temp.triggerClass=="x-form-date-trigger"){
		 				...
		 				if(temp.id.split("#")[1]=="startDate"){
		 					...
		 					jhy = tempValue;
		 			}else if(temp.id.split("#")[1]=="sqbm"){
		 				    sqbm = tempValue;
		 			}else if(temp.id.split("#")[1]=="is_cancel"){
		 				    is_cancel = tempValue;
		 			}else if (temp.id.split("#")[1]=="xqjhsqbh"){
		 					xqjhsqbh = tempValue;
	 	...
	 	**/
	 	...
	 	if(is_cancel != ""){
		 	url += "&is_cancel="+is_cancel;
	 	}
	 	if(xqjhsqbh != ""){
		 	url += "&xqjhsqbh="+xqjhsqbh;
	 	}
	 	if(wzda != ""){
		 	url += "&wzda="+wzda;
	 	}
				...
				if (sType == "textField") {
					...
					oSearchField = new Ext.form.TriggerField( {
						...
						onTriggerClick:function(){
								...
								var result = showIFModule(url,"工程项目选择","true",300,500,obj);
					...
					oSearchField = new Ext.form.ComboBox({  
						...
						store:new Ext.data.Store({
							...
							autoLoad : true, 
							proxy: new Ext.data.ScriptTagProxy({url : link6}),
