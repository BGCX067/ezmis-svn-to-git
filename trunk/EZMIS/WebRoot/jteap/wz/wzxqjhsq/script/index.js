...
function btnCancel_Click() {
		var select = rightGrid.getSelectionModel().getSelected();
		if (select.get('STATUS') != '1' && select.get('STATUS') != '0') {
			alert('此异动报告不符合作废标准，无法作废');
			return;
		}
		if (window.confirm('是否作废此需求计划申请？')) {
			Ext.Ajax.request({
						url : link7,
						method : 'POST',
						params : {
							...
							token : select.get('TOKEN'),
							curTaskName : select.get('CURRENT_TASKNAME')
						},
						success : function(ajax) {
							var responseText = ajax.responseText;
							var obj = Ext.decode(responseText);
