	...
	this.on("celldblclick", function(grid, rowIndex, columnIndex, e) {
				...
				if (node.id == 'dcl') {
					...
					**/
					...
					var url = link9;
					var windowUrl = link8 + "?pid=" + select.get("PROCESSINSTANCE_");
					var args = "url|" + windowUrl + ";title|" + '查看流程';
					var retValue = showModule(url, "yes", 800, 600, args);
					rightGrid.getStore().reload();
				} else {
					var url = link9;
					var windowUrl = link10 + "?pid=" + select.get("PROCESSINSTANCE_") + "&status=false";
					var args = "url|" + windowUrl + ";title|" + '查看流程';
					var retValue = showModule(url, "yes", 800, 600, args);
				}
			});
...
Ext.extend(RightGrid, Ext.grid.GridPanel, {
	...
	getDefaultDS : function(status) {
		...
		if (status == 'dcl') {
			url = link2;
			procFields = ['ID', 'XQJHSQBH', 'GCLB', 'GCXM', 'SQBMMC','FLOW_STATUS', 'CZYXM',
					'SQSJ', 'STATUS', "FLOW_TOPIC", "TASKTODOID", "FLOW_NAME",
