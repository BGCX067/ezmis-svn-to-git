...
RightGrid=function(){
	...
	this.on("celldblclick",function(grid, rowIndex, columnIndex, e){
		...
		if(select){
			...
			var params = 'crklbmc|'+$dictKey("CKRZLB",select.json.crklb)+';'+
...
Ext.extend(RightGrid, Ext.grid.GridPanel, {
	...
	getColumnModel:function(){
	    var cm=new Ext.grid.ColumnModel([
		    	//this.sm,
				{id:'crkdh',header: "出库单编号", width: 100, sortable: true, dataIndex: 'crkdh'},
		    	{id:'crksj',header: "出库时间", width: 100, sortable: true, dataIndex: 'crksj',
		    	 renderer:function(value,metadata,record,rowIndex,colIndex,store){
						var dt = formatDate(new Date(value["time"]),"yyyy-MM-dd"); 
						return dt;         
					}
		    	},
		    	{id:'crklb',header: "出库类别", width: 100, sortable: true, dataIndex: 'crklb',
		    		renderer:function(value,metadata,record,rowIndex,colIndex,store){
						return $dictKey("CKRZLB",value);
					}
		    	},
