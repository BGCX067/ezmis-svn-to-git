...
var formSn = "TB_WZ_PROJ";
//添加库位信息
function btnAddModule_Click(){
	if(!leftTree.getSelectionModel().getSelectedNode() || !leftTree.getSelectionModel().getSelectedNode().leaf){
		alert("请选择工程类别");
		return ;
	}
	var projcatcode = leftTree.getSelectionModel().getSelectedNode().id;
	var projcatname = leftTree.getSelectionModel().getSelectedNode().text;
	var pid = '';
	if(kwTreeGrid.getSelectionModel().getSelected())
		pid = kwTreeGrid.getSelectionModel().getSelected().json.id;
	var kwlj = kwTreeGrid.root_title+"/";
	var params = "projcatcode|"+projcatcode+";"+
							 "projcatname|"+projcatname+";"+
							 "pid|"+pid;
	var result=showModule(link20,true,480,235,params);
//	if(result=="" || result==null){
		//rightGrid.getStore().reload();
	//var url = contextPath + "/jteap/form/eform/eformRec.jsp?formSn="+formSn+"&projcatcode="+ckid+"&sjid="+kwid;
...
function btnModiModule_Click(){
	...
	if(select){
		...
		var params = "projcatcode|"+projcatcode+";"+
							 "projcatname|"+projcatname;
	var result=showModule(link20+"&docid="+select.json.id,true,480,235,params);	
...
function btnDelModule_Click(){
	var url = contextPath + "/jteap/wz/gcxmgl/ProjAction!delProjAction.do";
