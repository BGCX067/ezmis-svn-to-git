...
//				{id:'id',header: "ID", width: 100, sortable: true, dataIndex: 'id'},
				...
				{id:'dbxz',header: "调出性质", width: 100, sortable: true, dataIndex: 'dbxz'},
				{id:'dbqdw',header: "调出前单位", width: 100, sortable: true, dataIndex: 'dbqdw'},
				{id:'dbhdw',header: "调出后单位", width: 100, sortable: true, dataIndex: 'dbhdw'},
				{id:'personCzr.userName',header: "操作人", width: 80, sortable: false, dataIndex: 'personCzr.userName',
					renderer:function(value,metadata,record,rowIndex,colIndex,store){
						return value;
					}
				},
				{id:'personYsr.userName',header: "批准人", width: 80, sortable: false, dataIndex: 'personYsr.userName',
					renderer:function(value,metadata,record,rowIndex,colIndex,store){
						return value;
					}
				},
				{id:'personThr.userName',header: "复核人", width: 80, sortable: false, dataIndex: 'personThr.userName',
					renderer:function(value,metadata,record,rowIndex,colIndex,store){
						return value;
					}
				},
				{id:'personCwfzr.userName',header: "经办人", width: 80, sortable: false, dataIndex: 'personCwfzr.userName',
					renderer:function(value,metadata,record,rowIndex,colIndex,store){
						return value;
					}
				},
				{id:'dbsj',header: "调出时间", width: 150, sortable: true, dataIndex: 'dbsj',
				    renderer:function(value,metadata,record,rowIndex,colIndex,store){
				    	if(!value) return;
	...
	enableCgjh:function(select){
        ...
        callback :function(r,options,success){ 
            if(success){
            	var flag = false;
            	var str_ids = "";
            	var tmpRecords = new Array();
            	for (var index = 0; index < r.length; index++) {
            		if(str_ids.indexOf(r[index].data.wzda.id)<0){
            			tmpRecords.push(r[index]);
            			str_ids = str_ids + r[index].data.wzda.id + ",";
            		}
            	}
				...
				for(var i=0;i<r.length;i++){   
					...
					for (var j = 0; j < tmpRecords.length; j++) {
						if(record.json.wzda.id == tmpRecords[j].json.wzda.id){
							var dqkc = tmpRecords[j].data.wzda.dqkc;
							var yfpsl = tmpRecords[j].data.wzda.yfpsl;
							var yxkc = dqkc - yfpsl;
							if(cgsl>yxkc){
								...
								tmpRecords[j].get('wzda').yfpsl= yfpsl+cgsl;
								//tmpRecords[j].set('wzda.yfpsl',yfpsl+cgsl);
								//tmpRecords[j].commit();
								flag = true;
							}
						}
