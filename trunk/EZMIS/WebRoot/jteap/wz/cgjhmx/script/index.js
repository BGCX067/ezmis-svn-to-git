...
var searchAllFs="物资名称#wzdagl.wzmc#wzdagl,型号规格#wzdagl.xhgg#textField,采购计划编号#cgjhgl.bh#textField,申请部门#sqbmmc#depts,计划员#cgjhgl.jhy#comboJhy,采购员#cgy#comboCgy,状态#zt#comboZt,制定时间起始#zdStart#dateField,制定时间终止#zdEnd#dateField,采购方向#cgfx#comboCgfx,生效时间起始#sxStart#dateField,生效时间终止#xsEnd#dateField".split(",");
//查询面板中默认显示的条件，格式同上
var searchDefaultFs="物资名称#wzdagl.wzmc#wzdagl,型号规格#wzdagl.xhgg#textField,采购计划编号#cgjhgl.bh#textField,申请部门#sqbmmc#depts,计划员#cgjhgl.jhy#comboJhy,采购员#cgy#comboCgy,状态#zt#comboZt,制定时间起始#zdStart#dateField,制定时间终止#zdEnd#dateField,采购方向#cgfx#comboCgfx,生效时间起始#sxStart#dateField,生效时间终止#xsEnd#dateField".split(",");
...
function exportSelectedExcel(grid){
	...
	if(select.length > 0){
		...
		//window.open(path);
		var path = contextPath + "/jteap/wz/cgjhmx/CgjhmxAction!exportSelectedExcelAction.do?";
