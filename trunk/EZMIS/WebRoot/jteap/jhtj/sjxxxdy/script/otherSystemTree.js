 ...
 rootNode.on("load",function(){
	...
	AjaxRequest_Sync(link7,param,function(request){
		...
		if(responseObj.success){
			...
			var sid=data.sid;
			var myNode=leftTree.getNodeById(sid);
			alert(myNode.id);
			myNode.select();
		}
	});*/
	//selNode.select();
	//alert(myNode.isSelected());
	//
});
...
LeftTree= function(){
    ...
    this.getSelectionModel().on("selectionchange",function(oSM,oNode){	
		...
		if(oNode!=null&&oNode.isLeaf()){
			var sysid=oNode.parentNode.id;
			var sjyid=oNode.id;
			otherSystemForm.loadData(sysid,sjyid);
			otherSystemForm.setDefaultData(sjyid);
