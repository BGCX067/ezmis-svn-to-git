...
//		TANGER_OCX_OBJ.BeginOpenFromURL("ReadDocServlet?templateId=" + docid);
	...
	else
	{
		...
		//TANGER_OCX_OBJ.CreateNew("Excel.Sheet");
		alert("Excel 控件加载失败,请确认已关闭操作系统中所有excel进程!");
...
function TANGER_OCX_SetReadOnlyPwd(boolvalue,pwd){
	var i;
	try
	{
		if (boolvalue) TANGER_OCX_OBJ.IsShowToolMenu = false;
		with(TANGER_OCX_OBJ.ActiveDocument){
			if (TANGER_OCX_OBJ.DocType == 1) {//word
				...
				for(i=1;i <= Application.Sheets.Count;i++){
					if(boolvalue){
						Application.Sheets(i).Protect(pwd,true,true,true);
					}else{
						Application.Sheets(i).Unprotect(pwd);
					}
				}
				if(boolvalue){
					Application.ActiveWorkbook.Protect(pwd,true);					
				}else{
					Application.ActiveWorkbook.Unprotect(Protect);
				}
			}
		}
	}
...
function setExcelConfig(){
	if(document.TANGER_OCX.GetOfficeVer() ==11){//当前office版本不是office2007
		//用于设置批注显示状态为显示红标状态
		document.TANGER_OCX.ActiveDocument.Application.DisplayCommentIndicator = 0;
		//document.TANGER_OCX.ActiveDocument.Application.DisplayCommentIndicator = 1;
		//alert(1);
	}
}