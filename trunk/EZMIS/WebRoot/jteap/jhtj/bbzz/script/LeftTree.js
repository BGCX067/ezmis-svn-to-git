...
LeftTree= function(){
    ...
    this.getSelectionModel().on("selectionchange",function(oSM,oNode){
		...
		var btnStatusCatalog=mainToolbar.items.get("btnStatusCatalog");
		...
		if(oNode==null){
			...
			if(btnStatusCatalog) btnStatusCatalog.setDisabled(true);
    		...
    		if(oNode.isRootNode()){
	    		...
	    		var type=oNode.attributes.type;
	    		if(type==2){
	    			if(btnAddCatalog) btnAddCatalog.setDisabled(false);
	    			var url=link4+"?bbindexid="+oNode.id;
