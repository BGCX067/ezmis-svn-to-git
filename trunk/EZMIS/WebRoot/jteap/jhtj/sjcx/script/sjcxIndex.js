...
function createTable(sjlx,sjlxCn,ywflList,tableName,updateData){
	var result='<ul><li>'+sjlxCn+'<input type="checkbox" id="'+sjlx+'Check" onclick="checkAllchecked('+tableName+',this);"></li></ul>';
	...
	for(var i=0;i<ywflList.length;i++){
		...
		if(flag==0){
			table=table+"<tr><td class='LM01Txt'><input type='checkbox' id='"+item.item+"Check'/>"+item.iname+"<input type='hidden' id='"+item.item+"' value='"+item.item+","+item.iname+"'/></td>";
			flag++;
		}else{
			table=table+"<td class='LM01Txt'><input type='checkbox' id='"+item.item+"Check'/>"+item.iname+"<input type='hidden' id='"+item.item+"' value='"+item.item+","+item.iname+"'/></td>";
		}
		bFlag++;
	}
