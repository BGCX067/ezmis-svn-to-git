var TjAppIOForm = function(id,sysid,isSql,strSql, isEdit) {
	...
	function initTable(sql){
		...
		param.sql=sql;
		param.sysid=sysid;
		AjaxRequest_Sync(link14, param, function(req) {
			var responseText=req.responseText;	
 			var responseObject=Ext.util.JSON.decode(responseText);
 			if(responseObject.success){
 				var cols=responseObject.list;
 				var cm=buildCm(cols);
 				var ds=buildDs(cols,sql,sysid);
	...
	function buildDs(cols,sql){
		...
		var ds = new Ext.data.Store({
	        proxy: new Ext.data.HttpProxy({
	            url: link15+"?sql="+URLencode(sql)+"&sysid="+sysid
	        }),
...
/**
	...
	TjAppIOForm.superclass.constructor.call(this, {
		width : '100%',
		height : 425,
		modal : true,
		autoScroll : true,
...
Ext.extend(TjAppIOForm, Ext.Panel, {});
