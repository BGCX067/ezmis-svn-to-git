...
function fillTableFieldsFromServer(fields){
	...
	var oTable=xml_createTable(currSystemId,sTableName,sTableCName);
	...
	for(var i=0;i<fields.length;i++){
		...
		var sChkTd="<td><input "+sChecked+" onclick='onFieldChecked();' type='checkbox' name='chk_field_"+field.fname+"' style='border:0px'/>"+field.fname+"</td>";
		var sInputTd="<td><input onchange='onFieldNameChanged();' type='text' name='input_field_"+field.fname+"' value='"+field.fname+"' style='border:0px'/></td>";
		
		str+="<tr>"+sChkTd+sInputTd+"<td>"+field.ftype+"</td></tr>";
		var oField=xml_createField(oTable,field.fname,field.fname,field.ftype);
		if(sChecked!=''){
...
function onSelectTable(selectRows){
	...
	if(oTable!=null){
		...
		AjaxRequest_Sync(link9,loadParam,function(request){
