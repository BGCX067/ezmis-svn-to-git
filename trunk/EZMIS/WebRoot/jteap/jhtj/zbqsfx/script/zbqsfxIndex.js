...
function createTable(sjlx,sjlxCn,ywflList,tableName,updateData){
	...
	for(var i=0;i<ywflList.length;i++){
		...
		if(flag==0){
			table=table+"<tr><td class='LM01Txt'><a href='${contextPath}/zbqsfxAction!showChartAction.do?kid="+kid+"&item="+item.item+"&iname="+item.iname+"&flflag="+flflag+"'>"+item.iname+"</a></td>";
			flag++;
		}else{
			table=table+"<td class='LM01Txt'><a href='${contextPath}/zbqsfxAction!showChartAction.do?kid="+kid+"&item="+item.item+"&iname="+item.iname+"&flflag="+flflag+"'>"+item.iname+"</a></td>";
		}
		bFlag++;
	}
