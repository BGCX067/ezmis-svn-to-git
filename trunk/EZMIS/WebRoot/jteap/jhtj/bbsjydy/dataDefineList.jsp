				...
				<TABLE border="0" width='100.5%'  height="100%">			
								...
								<div id="body" class="webfx-columnlist-body" style="width: 100%;height: 100%">
									<table cellspacing="0" cellpadding="0" style="width: 100%;">
										...
										<c:forEach items="${tables}" var="table">
											<tr>
												<td>
													<input onclick="onTableChecked();" type="checkbox" name="chk_table_${table.tableName}" style="border: 0px"/>${table.tableName}
												</td>
												<td>
													<input type="text" name="input_table_${table.tableName}" value="<c:if test="${table.tableName!=null}">${table.tableName}</c:if>" style="border: 0px" onchange="onTableNameChanged();" onclick="list1.selectRow(this.parentNode.parentNode.rowIndex);">
												</td>
											</tr>						
										</c:forEach>
									</table>
					...
					<TR height="30px">
						<TD align='right' valign="middle" colspan=4>
							<form name="myForm" target="next" action="${contextPath}/jteap/jhtj/bbsjydy/bbIOAction!showLastPageAction.do" method="post">
							<input type="hidden" name="xmlField" value=""/>
							<input type="hidden" name="bbindexid" value=""/>
