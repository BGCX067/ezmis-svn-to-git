...
<html>
  ...
  <script language="javascript">
			...
			var k=0;
			function init(){
				var initValue="${requestScope.selectKey}";//初始化数据
				if(initValue==""){
					alert("初始化接口失败!");
				}else{
					//具体显示的字段
					...
					for(var i=0;i<datas.length;i++){
						...
						//是机组就特别处理
						if(items[0]=="JZ"){
							var startStr=items[1].substring(0,1);
							var start=parseInt(items[1].substring(1));
							var end=parseInt(items[2].substring(1));
							for(var j=start;j<=end;j++){
								result=result+"<OPTION VALUE="+startStr+j+">"+startStr+j+"</OPTION>";
							}
						}else{
							if(items[0]!="SRQ"&&items[0]!="ERQ"){
