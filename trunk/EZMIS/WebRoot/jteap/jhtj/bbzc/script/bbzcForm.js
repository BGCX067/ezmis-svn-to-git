var BbzcForm = function(id,parentId,isEdit) {
	var filterName = /^\s*[A-Z0-9_]{1,30}\s*$/;

	var flbmCell=new Ext.app.UniqueTextField( {
		id : 'flbm',
		name : 'flbm',
		fieldLabel : '分类编码',
		allowBlank : false,
	...
	var flmcCell=new Ext.form.TextField( {
		disabled : false,
		width : 150,
		maxLength : 50,
		allowBlank : false,
	...
	this.loadData = function() {
		if(id!=""){
			...
			AjaxRequest_Sync(link7,loadParam,function(request){
				...
				flbmCell.setValue(data.flbm);
				flbmCell.setDisabled(true);
				flmcCell.setValue(data.flmc);
				sortnoCell.setValue(data.sortno);
			});
		}
	}

	BbzcForm.superclass.constructor.call(this, {
		width : '100%',
		height : 420,
		modal : true,
		autoScroll : true,
