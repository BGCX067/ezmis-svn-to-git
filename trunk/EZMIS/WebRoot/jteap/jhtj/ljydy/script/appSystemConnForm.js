var AppSystemConnForm = function(dbsId,isEdit) {
	...
	this.loadData = function() {
		if(dbsId!=""){
			if(isEdit!=""){
				...
				combLinkTypeName.setDisabled(true);
				txtDriveName.setDisabled(true);
				txtAddress.setDisabled(true);
				txtDbName.setDisabled(true);
				txtPort.setDisabled(true);
				txtUserName.setDisabled(true);
				txtPassword.setDisabled(true);
				txtLinkString.setDisabled(true);
				sortnoCell.setDisabled(true);
				$("testButton").style.display="none";
				$("saveButton").style.display="none";
			}
			
			var param={};
			AjaxRequest_Sync(link9 + "?id=" + dbsId,param,function(request){
				...
				if(responseObj.success){
					...
					txtDbsName.setDisabled(true);
					//加入修改初始化连接
					combStore.each(function(record){
						var typeid=record.data.typeid;
						var curId=combLinkTypeName.getValue();
						if(typeid==curId){
							addDbsWindow.dbUrl = record.data.url;
						};
					});
