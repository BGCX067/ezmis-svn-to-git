	 		...
	 		if(oItem.hidden==false){
	 			...
	 			if(tempValue!=null&&tempValue!=""){
		 			...
		 			if(temp.triggerClass=="x-form-date-trigger"){
		 				tempValue = formatDate(tempValue,"yyyy-MM");
		 				queryParamsSql+="to_char("+temp.id.split("#")[1]+",'yyyy-mm')='"+tempValue+"' and ";
		 			//其他字段模糊查询
		 			}else{
		 				queryParamsSql+=temp.id.split("#")[1]+" like '%"+tempValue+"%' and ";
		 			}
	 			}
	 		};
	 	});
...
//	 	var txtWidth=30;
		...
		//日期类型
			...
			var storeHyyt=  new Ext.data.SimpleStore({
				fields: ["retrunValue", "displayText"],
				data: [['民主生活记录','民主生活记录'],['政治学习','政治学习'],['班委会','班委会']]
			});
			//评级级别
			if(arrays[1] == 'hyzt'){
