...
SearchPanel = function(config) {
	...
	this.searchClick = function() {
					...
					if (oItem.hidden == false) {
						...
						if (tempValue != null && tempValue != "") {
							if (temp.id.split("#")[1] == "SBMC") {
								...
								queryParamsSql += " a." + temp.id.split("#")[1] + " like '"
										+ encodeURIComponent("%" + tempValue + "%") + "' and ";
							} else if (temp.id.split("#")[1] == "STATUS") {
								queryParamsSql += " a." + temp.id.split("#")[1] + " like '"
										+ encodeURIComponent("%" + tempValue + "%") + "' and ";
							} else if (temp.id.split("#")[1] == "POST_PERSON") {
								queryParamsSql += " b." + temp.id.split("#")[1] + " like '"
										+ encodeURIComponent("%" + tempValue + "%") + "' and ";
		...
		var param = "&queryParamsSql=" + queryParamsSql.substring(0, queryParamsSql.length - 5);
	...
	this.clearSearch = function() {
		...
		var node = leftTree.getSelectionModel().getSelectedNode();
		rightGrid.changeToListDS(node.id, "");
		rightGrid.getStore().reload();
	};
