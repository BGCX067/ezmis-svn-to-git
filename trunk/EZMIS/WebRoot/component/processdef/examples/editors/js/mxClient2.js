...
var mxUtils =
{
    ...
    saveAs:function(content)
    {
        ...
        try
        {
            if (mxClient.IS_NS)
            {
                ...
                {
                    alert('Permission to save document denied.');
                }
            }
            else
            {
    ...
    getScrollOrigin:function(node)
    {
        ...
        if (node != null)
        {
            while (node.parentNode != null && node.parentNode != b && node.parentNode != d)
            {
    ...
    makeDraggable:function(element, graph, funct, dragElement)
    {
        mxEvent.addListener(element, 'mousedown', function(evt)
        {
            if (!mxEvent.isConsumed(evt))
            {
                ...
                var dragHandler = function(evt)
                {
                    ...
                    sprite.style.left = (evt.clientX + origin.x) + 'px';
                    sprite.style.top = (evt.clientY + origin.y + mxConstants.TOOLTIP_VERTICAL_OFFSET) + 'px';
...
var mxClipboard =
{
    ...
    copy:function(graph, cells)
    {
        ...
        if (cells != null && cells.length > 0)
        {
            ...
            {
                var cell = cells[i];
                if (graph.canExport(cell))
                {
                    result.push(cell);
                    mxClipboard.parents.push(model.getParent(cell));
                }
            }
    ...
    paste:function(graph)
    {
        ...
        if (cells != null)
        {
            ...
            var model = graph.getModel();
            var pasted = new Array();
            graph.dispatchEvent('beforePaste', graph, cells, parents, parent);
            model.beginUpdate();
            try
            {
                for (var i = 0; i < parents.length; i++)
                {
                    if (graph.canImport(cells[i]))
                    {
                        pasted.push(cells[i]);
                        var geo = model.getGeometry(cells[i]);
                        var pstate = graph.getView().getState(parents[i]);
                        ...
                        else
                        {
                            ...
                            if (geo != null)
                            {
                                ...
                                var tmp = graph.getSwimlaneAt(geo.x + mxClipboard.STEPSIZE, geo.y + mxClipboard.STEPSIZE);
                                if (tmp != null)
                                {
                                    currentParent = tmp;
                                    var pstate = graph.getView().getState(currentParent);
                                    if (pstate != null)
                                    {
                                        geo.x -= pstate.origin.x;
                                        geo.y -= pstate.origin.y;
                            ...
                            model.add(currentParent, cells[i]);
                        }
                    }
                }
                graph.move(cells, mxClipboard.STEPSIZE, mxClipboard.STEPSIZE);
                graph.dispatchEvent('paste', graph, pasted);
            }
            finally
            {
                model.endUpdate();
            ...
            graph.dispatchEvent('afterPaste', graph, pasted);
...

{
    ...
    mxShape.prototype.configureVmlShape = function(node)
    {
        ...
        {
            mxUtils.release(this.strokeNode);
...

{
    ...
    mxPolyline.prototype.create = function()
    {
        ...
        else if (this.dialect == mxConstants.DIALECT_STRICTHTML || (this.dialect == mxConstants.DIALECT_PREFERHTML && this.points.length > 0))
        {
            node = document.createElement('DIV');
            this.configureHtmlShape(node);
            node.style.borderStyle = 'none';
...

{
    ...
    mxLine.prototype.redrawVml = function()
    {
        ...
        if (this.style[mxConstants.STYLE_HORIZONTAL] == false)
        {
...

{
    ...
    mxConnector.prototype.redrawVml = function()
    {
        if (this.strokeNode != null)
        {
            ...
            this.strokeNode.setAttribute('startarrowwidth', width);
            this.strokeNode.setAttribute('startarrowlength', length);
            this.strokeNode.setAttribute('endarrowwidth', width);
            this.strokeNode.setAttribute('endarrowlength', length);
            this.updateVmlShape(this.node);
        }
    };
    mxConnector.prototype.createSvg = function()
    {
        ...
        {
            this.start = this.createSvgMarker(this.startArrow, color);
            g.appendChild(this.start);
        }
        if (this.endArrow != null)
        {
            this.end = this.createSvgMarker(this.endArrow, color);
    ...
    mxConnector.prototype.createSvgMarker = function(type, color)
    {
        var node = document.createElementNS(mxConstants.NS_SVG, 'path');
        node.setAttribute('stroke', color);
        if (type != 'open')
        {
            node.setAttribute('fill', color);
    ...
    mxConnector.prototype.redrawSvg = function()
    {
        ...
        if (this.points != null && this.points[0] != null)
        {
            if (this.start != null)
            {
                ...
                this.startOffset = this.redrawSvgMarker(this.start, this.startArrow, p0, pe, mxConstants.DEFAULT_MARKERSIZE);
            }
            if (this.end != null)
            {
                ...
                this.endOffset = this.redrawSvgMarker(this.end, this.endArrow, p0, pe, mxConstants.DEFAULT_MARKERSIZE);
            }
        }
        this.updateSvgShape(this.innerNode);
        this.pipe.setAttribute('d', this.innerNode.getAttribute('d'));
    ...
    mxConnector.prototype.redrawSvgMarker = function(node, type, p0, pe, size)
    {
        var offset = null;
        var dx = pe.x - p0.x;
        var dy = pe.y - p0.y;
...

{
    ...
    mxSwimlane.prototype.createSvg = function()
    {
        ...
        if (color != null)
        {
            ...
            this.separator.style.cursor='row-resize';
            node.appendChild(this.separator);
        }
        if (this.image != null)
        {
...

{
    ...
    mxGraphLayout.prototype.getVertexBounds = function(cell)
    {
        ...
        if (this.useBoundingBox)
        {
            ...
            {
                var scale = this.graph.getView().scale;
                var tmp = state.text.boundingBox.clone();
                tmp.x /= scale;
                tmp.y /= scale;
                tmp.width /= scale;
                tmp.height /= scale;
...
{
    function mxCompactTreeLayout(graph, horizontal)
    {
        mxGraphLayout.call(this, graph);
        this.horizontal = (horizontal != null) ? horizontal : true;
    }
    ...
    mxCompactTreeLayout.prototype.execute = function(parent)
    {
        var model = this.graph.getModel();
        var root = null;
        if (this.graph.getOutgoingEdges(parent, model.getParent(parent)).length > 0)
        {
            root = parent;
        }
        ...
        else
        {
            ...
            if (roots.length > 0)
            {
                ...
                {
                    if (!this.isVertexIgnored(roots[i]) && this.graph.getOutgoingEdges(roots[i]).length > 0)
                    {
                        root = roots[i];
                        break;
                    }
    ...
    mxCompactTreeLayout.prototype.dfs = function(cell, parent, visited)
    {
        ...
        if (cell != null && visited[id] == null && !this.isVertexIgnored(cell))
        {
            ...
            var model = this.graph.getModel();
            var prev = null;
            var out = this.graph.getOutgoingEdges(cell, parent);
            for (var i = 0; i < out.length; i++)
            {
                var edge = out[i];
                if (!this.isEdgeIgnored(edge))
                {
                    var target = this.graph.getView().getVisibleTerminal(edge, false);
...

{
    ...
    mxGraphModel.prototype.cloneCellImpl = function(cell, mapping, includeChildren)
    {
        ...
        mapping[mxCellPath.create(cell)] = clone;
        if (includeChildren)
        {
            var childCount = this.getChildCount(cell);
            for (var i = 0; i < childCount; i++)
    ...
    mxGraphModel.prototype.restoreClone = function(clone, cell, mapping)
    {
        ...
        if (source != null)
        {
            var tmp = mapping[mxCellPath.create(source)];
            if (tmp != null)
            {
        ...
        if (target != null)
        {
            var tmp = mapping[mxCellPath.create(target)];
            if (tmp != null)
            {
...

{
    ...
    mxCell.prototype.is = function(type, attr, value)
    {
        ...
        var nodeName = (userObject != null) ? userObject.nodeName : null;
...

{
    ...
    mxCellRenderer.prototype.resolveColor = function(state, field, key)
    {
        ...
        else if (value == 'swimlane')
        {
            ...
            referenced = this.getSwimlane(referenced);
    ...
    mxCellRenderer.prototype.getLabelValue = function(state)
    {
        ...
        if (!graph.isHtmlLabel(state.cell) && (value != null && isNaN(value.nodeType)) && mxClient.IS_IE)
        {
    ...
    mxCellRenderer.prototype.createLabel = function(state)
    {
        ...
        if (state.style[mxConstants.STYLE_FONTSIZE] > 0 || state.style[mxConstants.STYLE_FONTSIZE] == null)
        {
            ...
            var swimlane = graph.isSwimlane(state.cell);
            var isForceHtml = (graph.isHtmlLabel(state.cell) || (value != null && !isNaN(value.nodeType))) && graph.dialect == mxConstants.DIALECT_SVG;
            var isRotate = (swimlane && state.style[mxConstants.STYLE_HORIZONTAL] == true) || (!swimlane && state.style[mxConstants.STYLE_HORIZONTAL] == false)
...
var mxEdgeStyle =
{
    ...
    {
        var graph = state.view.graph;
        var segment = 30 * state.view.scale;
...

{
    ...
    mxGraphView.prototype.model = null;
    ...
    mxGraphView.prototype.clear = function(cell, force, recurse)
    {
        ...
        if (recurse && (force || cell != this.currentRoot))
        {
            ...
            for (var i = 0; i < childCount; i++)
            {
                this.clear(this.model.getChildAt(cell, i), force);
            }
        }
        else
        {
    ...
    mxGraphView.prototype.validate = function(cell)
    {
        ...
        this.bounds = this.validatePoints(null, null, cell);
    ...
    mxGraphView.prototype.validateBounds = function(parent, parentState, cell)
    {
        ...
        if (state != null && state.invalid)
        {
            ...
            else if (cell != this.currentRoot && parent != null && parentState != null)
            {
        ...
        if (state != null && (!this.graph.isCellCollapsed(cell) || cell == this.currentRoot))
        {
            ...
            {
                var child = this.model.getChildAt(cell, i);
                this.validateBounds(cell, state, child);
            }
        }
    };
    mxGraphView.prototype.validatePoints = function(parent, parentState, cell)
    {
        var minX = null;
        var minY = null;
        var maxX = 0;
        ...
        if (state != null)
        {
            if (state.invalid)
            {
                ...
                if (this.model.isEdge(cell))
                {
                    ...
                    {
                        var p = this.model.getParent(src);
                        var pstate = this.getState(p);
                        this.validatePoints(p, pstate, src);
                    }
                    var trg = this.getVisibleTerminal(cell, false);
                    if (trg != null && !this.model.isAncestor(trg, cell))
                    {
                        var p = this.model.getParent(trg);
                        var pstate = this.getState(p);
                        this.validatePoints(p, pstate, trg);
                    }
                ...
                else if (geo != null && geo.relative && this.model.isEdge(parent))
                {
        ...
        if (state != null && (!this.graph.isCellCollapsed(cell) || cell == this.currentRoot))
        {
            ...
            {
                var child = this.model.getChildAt(cell, i);
                var bounds = this.validatePoints(cell, state, child);
    ...
    mxGraphView.prototype.updatePoints = function(state, points)
    {
        if (state != null)
        {
            ...
            var edgeStyle = (src != null && src == trg) ? state.style[mxConstants.STYLE_LOOP] || this.graph.defaultLoopStyle :
...

{
    ...
    {
        cell = cell || this.getSelectionCell();
        this.toggleCellStyles(key, [cell]);
    };
            ...
            try
            {
                ...
                if (recurse)
                {
                    ...
                    if (children != null)
                    {
                        ...
                        {
                            this.removeEdges(children[i], true);
                        }
                    }
                }
            }
    ...
    mxGraph.prototype.updateSwimlaneSize = function(swimlane, geometry, pSize)
    {
        if (swimlane != null && geometry != null && pSize != null)
        {
            ...
            try
            {
                ...
                if (style[mxConstants.STYLE_HORIZONTAL])
                {
    ...
    mxGraph.prototype.move = function(cells, dx, dy, clone, target, evt)
    {
        ...
        if (clones != null && (dx != 0 || dy != 0 || clone || target != null))
        {
            ...
            try
            {
                ...
                else if (this.allowDanglingEdges)
                {
            ...
            try
            {
                if (target != null)
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < cells.length; i++)
                        {
                            if (cells[i] != target && (!this.model.maintainEdgeParent || !this.model.isEdge(cells[i])))
                            {
                                var parent = this.model.getParent(cells[i]);
                        ...
                        if (g != null)
                        {
                            ...
                            if (state != null && pstate != null)
                            {
                                ...
                                if (src != null && this.isDisconnectable(cells[i], src, true))
                                {
                                    ...
                                    if (src == null)
                                    {
                                        g.setTerminalPoint(new mxPoint((pts[0].x + dx) / scale - tr.x, (pts[0].y + dy) / scale - tr.y), true);
                                ...
                                {
                                    var trdId = mxCellPath.create(trg);
                                    while (trg != null && hash[trgId] == null)
                                    {
                                        trg = this.model.getParent(trg);
                                        trgId = mxCellPath.create(trg);
                                    ...
                                    {
                                        var n = pts.length - 1;
                                        g.setTerminalPoint(new mxPoint((pts[n].x + dx) / scale - tr.x, (pts[n].y + dy) / scale - tr.y), false);
    ...
    mxGraph.prototype.updateAlternateBounds = function(cell, g, willCollapse)
    {
        if (cell != null && g != null)
        {
            ...
            {
                var bounds = (this.collapseToPreferredSize) ? this.getPreferredSizeForCell(cell) || g : g;
    ...
    mxGraph.prototype.getCellValidationError = function(cell)
    {
        ...
        for (var i = 0; i < this.multiplicities.length; i++)
        {
            ...
            if (rule.source && cell.is(rule.type, rule.attr, rule.value) && ((rule.max == 0 && outCount > 0) || (rule.min == 1 && outCount == 0) || (rule.max == 1 && outCount > 1)))
            {
                error += rule.countError + '\n';
            }
            else if (!rule.source && cell.is(rule.type, rule.attr, rule.value) && ((rule.max == 0 && inCount > 0) || (rule.min == 1 && inCount == 0) || (rule.max == 1 && inCount > 1)))
            {
                error += rule.countError + '\n';
            }
        }
    ...
    {
        return cell != null && (!this.model.isEdge(cell) || this.connectableEdges) && this.model.isConnectable(cell);
    };
    ...
    mxGraph.prototype.findTreeRoots = function(parent, isolate)
    {
            ...
            for (var i = 0; i < childCount; i++)
            {
                ...
                if (this.model.isVertex(cell) && this.isCellVisible(cell))
                {
                    ...
                    for (var j = 0; j < conns.length; j++)
                    {
                        ...
                        var trg = this.view.getVisibleTerminal(conns[j], false);
                        if (src != trg)
                        {
                            if (src == cell)
                            {
...

{
    function mxOutline(graph, container)
    {
        ...
        if (mxClient.IS_SVG)
        {
            this.graph.getView().getCanvas().parentNode.setAttribute('shape-rendering', 'optimizeSpeed');
            this.graph.getView().getCanvas().parentNode.setAttribute('image-rendering', 'optimizeSpeed');
        }
        this.graph.setEnabled(false);
        this.graph.labelsVisible = false;
        this.graph.stylesheet = graph.getStylesheet();
        var self = this;
...

{
    ...
    {
        var error = '';
        if ((graph.model.getValue(source) != null && graph.model.getValue(target) != null) && ((this.source && source.is(this.type, this.attr, this.value)) || (!this.source && target.is(this.type, this.attr, this.value))))
        {
            if (this.source && (this.max == 0 || (sourceOut >= this.max)))
            {
                error += this.countError + '\n';
            }
            else if (!this.source && (this.max == 0 || (targetIn >= this.max)))
            {
                error += this.countError + '\n';
            }
            ...
            if (valid != null && valid.length > 0)
            {
                ...
                {
                    if (this.source && target.is(valid[j]))
                    {
                        isValid = this.validNeighborsAllowed;
                        break;
                    }
                    else if (!this.source && source.is(valid[j]))
                    {
                        isValid = this.validNeighborsAllowed;
                        break;
                    }
...

{
    ...
    mxGraphHandler.prototype.mouseMove = function(evt, cell)
    {
        ...
        if (this.cell != null)
        {
            ...
            {
                var dx = graph.snap(evt.clientX - this.startX);
                var dy = graph.snap(evt.clientY - this.startY);
                if (evt.shiftKey)
                {
                ...
                if (!graph.isCellSelected(target) && state != null && parent == null && model.getParent(this.cell) != target)
                {
    ...
    mxGraphHandler.prototype.mouseUp = function(evt, cell)
    {
        ...
        {
            var scale = graph.getView().scale;
            var clone = evt.ctrlKey && graph.isCloneable();
...

{
    ...
    mxPanningHandler.prototype.mouseUp = function(evt, cell)
    {
        ...
        {
            if (dx < this.graph.tolerance && dy < this.graph.tolerance && this.factoryMethod != null)
            {
...
{
    function mxTerminalMarker(graph, highlightColor, hotspot)
    {
        this.graph = graph;
        this.hotspot = (hotspot != null) ? hotspot : mxConstants.ACTIVE_REGION;
        this.highlightColor = highlightColor || mxConstants.HIGHLIGHT_COLOR;
        var strokeWidth = mxConstants.HIGHLIGHT_STROKEWIDTH;
        this.shape = new mxRectangleShape(new mxRectangle(0, 0, 0, 0), null, this.highlightColor, strokeWidth);
        this.shape.dialect = (this.graph.dialect != mxConstants.DIALECT_SVG) ? mxConstants.DIALECT_VML : mxConstants.DIALECT_SVG;
        this.shape.init(graph.getView().getOverlayPane());
        this.shape.node.style.background = '';
        this.shape.node.style.display = 'none';
        this.edgeShape = new mxPolyline([new mxPoint(0, 0),new mxPoint(0, 0)], this.highlightColor, strokeWidth);
        ...
        {
            graph.dispatchGraphEvent('mousedown', evt, self.cell);
        });
        mxEvent.addListener(this.shape.node, 'mousemove', function(evt)
        {
            graph.dispatchGraphEvent('mousemove', evt, self.cell);
        });
        mxEvent.addListener(this.shape.node, 'mouseup', function(evt)
        {
            graph.dispatchGraphEvent('mouseup', evt, self.cell);
        });
        ...
        this.graph.getView().addListener('scale', funct);
        this.graph.getView().addListener('translate', funct);
        this.graph.getView().addListener('down', funct);
        this.graph.getView().addListener('up', funct);
        this.graph.getModel().addListener('change', funct);
        this.sourcePoint = new mxPoint(0, 0);
        this.targetPoint = new mxPoint(0, 0);
    }
    ;
    mxTerminalMarker.prototype = new mxEventSource();
    mxTerminalMarker.prototype.constructor = mxTerminalMarker;
    mxTerminalMarker.prototype.graph = null;
    mxTerminalMarker.prototype.hotspot = null;
    mxTerminalMarker.prototype.highlighted = null;
    mxTerminalMarker.prototype.init = function(evt, cell)
    {
        if (cell != null)
        {
            this.highlight(null);
            this.source = cell;
            this.sourceState = this.graph.getView().getState(this.source);
            if (this.sourceState)
            {
    ...
    mxTerminalMarker.prototype.intersects = function(evt, state, jump)
    {
        var model = this.graph.getModel();
        var childCount = model.getChildCount(state.cell);
        if ((jump && childCount == 0) || this.hotspot == 0)
        {
            return true;
        }
        else
        {
            var w = Math.max(mxConstants.MIN_ACTIVE_REGION / 2, state.width * this.hotspot / 2);
            var h = Math.max(mxConstants.MIN_ACTIVE_REGION / 2, state.height * this.hotspot / 2);
            var region = new mxRectangle(state.getCenterX() - w, state.getCenterY() - h, 2 * w, 2 * h);
            if (this.graph.isSwimlane(state.cell))
            {
                var tmp = this.graph.getCellStyle(state.cell);
                if (tmp[mxConstants.STYLE_HORIZONTAL])
                {
                    region = new mxRectangle(state.x, state.getCenterY() - h, state.width, 2 * h);
                }
                else
                {
                    region = new mxRectangle(state.getCenterX() - w, state.y, 2 * w, state.height);
                }
            }
            var point = mxUtils.convertPoint(this.graph.container, evt.clientX, evt.clientY);
            return mxUtils.contains(region, point.x, point.y);
        }
    };
    mxTerminalMarker.prototype.updateTerminal = function(evt, cell, jump)
    {
        cell = this.getHighlightCell(evt, cell, jump);
        if (cell != null)
        {
            var state = this.graph.getView().getState(cell);
            if (this.intersects(evt, state, jump))
            {
                ...
                if (jump)
                {
                    ...
                    if (perimeter != null && this.sourceState)
                    {
                        ...
                        var bounds = state.getPerimeterBounds();
                        var point = perimeter(bounds, null, this.sourceState, false, next);
                    ...
                    {
                        var bounds = this.sourceState.getPerimeterBounds();
                        var point = this.sourcePerimeter(bounds, null, this.sourceState, false, this.targetPoint);
    ...
    mxTerminalMarker.prototype.highlight = function(state)
    {
        if (this.highlighted != state)
        {
            this.highlighted = state;
            this.dispatchEvent('highlight', this, state);
            if (state != null)
            {
                var shape = null;
                if (this.graph.model.isEdge(state.cell))
                {
                    this.hideHighlight(this.shape);
                    shape = this.edgeShape;
                    shape.points = state.absolutePoints;
                }
                else
                {
                    this.hideHighlight(this.edgeShape);
                    shape = this.shape;
                    shape.bounds = new mxRectangle(state.x - 2, state.y - 2, state.width + 4, state.height + 4);
                }
                shape.redraw();
                var color = this.getHighlightColor(state);
            ...
            this.dispatchEvent('afterHighlight', this, state);
        }
    };
    mxTerminalMarker.prototype.getHighlightColor = function(state)
    {
        return this.highlightColor;
    };
    mxTerminalMarker.prototype.getHighlightCell = function(evt, cell)
    {
        return(cell != null && ((this.graph.getModel().isEdge(cell) && this.graph.connectableEdges) || this.graph.getModel().isConnectable(cell))) ? cell : null;
    };
    mxTerminalMarker.prototype.hideHighlight = function(shape)
    {
        if (shape != null)
        {
    ...
    mxTerminalMarker.prototype.showHighlight = function(shape)
    {
        shape.node.style.display = 'inline';
        this.graph.getView().getOverlayPane().appendChild(shape.node);
    };
...

{
    ...
    mxConnectionHandler.prototype.init = function()
    {
        if (this.graph.container != null)
        {
            ...
            this.marker.getHighlightColor = function(state)
            {
                if (!self.active)
                {
                    self.error = (self.isValidSource(state.cell)) ? null : '';
                }
                else
                {
                    self.error = self.validateConnection(self.marker.source, state.cell);
                }
                if (self.error != null)
                {
                    return mxConstants.INVALID_CONNECT_TARGET_COLOR;
                }
                else if (self.active)
                {
                    return mxConstants.CONNECT_TARGET_COLOR;
                }
                return mxTerminalMarker.prototype.getHighlightColor.apply(this, arguments);
            };

            var previous = this.marker.getHighlightCell;
            this.marker.getHighlightCell = function(evt, cell)
            {
                if (cell == null || !self.graph.model.isEdge(cell) || self.graph.connectableEdges)
                {
    ...
    {
        if (this.isEnabled() && this.graph.isEnabled() && !mxEvent.isForceMarqueeEvent(evt) && ((this.icons == null) || (this.icons != null && this.icon != null)) && index == null)
        {
            this.error = (this.marker.cell != null && this.isValidSource(this.marker.cell)) ? null : '';
            if (this.error == null)
            {
                this.marker.init(evt, this.marker.cell);
                this.shape.redraw();
    ...
    mxConnectionHandler.prototype.mouseMove = function(evt, cell, index)
    {
        ...
        {
            if (cell == null && !this.createTarget)
            {
                this.error = '';
            }
            else if (cell != this.marker.cell)
            {
                this.error = null;
            }
            this.marker.updateTerminal(evt, cell, this.active);
            if (this.active)
            {
            ...
            {
                var state = this.graph.getView().getState(this.marker.cell);
    ...
    mxConnectionHandler.prototype.mouseUp = function(evt, cell)
    {
        if (this.active)
        {
            ...
            if (this.error == null)
            {
                ...
                var dx = this.graph.tolerance + 1;
                var dy = this.graph.tolerance + 1;
                if (source == target)
                {
                    dx = Math.abs(evt.clientX - this.startX);
                    dy = Math.abs(evt.clientY - this.startY);
                }
                if (dx > this.graph.tolerance || dy > this.graph.tolerance)
                {
                    this.connect(source, target, evt);
                }
                else
                {
    ...
    mxConnectionHandler.prototype.connect = function(source, target, evt)
    {
        if (source != null && (target != null || this.createTarget || this.graph.allowDanglingEdges))
        {
            ...
            if (edge != null)
            {
                ...
                try
                {
                    var geo = this.graph.model.getGeometry(edge);
                    if (geo == null)
                    {
                        geo = new mxGeometry();
                        geo.relative = true;
                    ...
                    {
                        target = this.createVertex(evt, source);
                        if (target != null)
                        {
                            this.graph.getModel().add(parent, target);
                        }
                    }
                    if (target == null)
                    {
                        var pt = this.marker.targetPoint.clone();
                        pt.x /= this.graph.getView().scale;
                        pt.y /= this.graph.getView().scale;
                        pt.x -= this.graph.getView().translate.x;
                        pt.y -= this.graph.getView().translate.y;
                        geo.setTerminalPoint(pt);
                    }
    ...
    mxConnectionHandler.prototype.createVertex = function(evt, source)
    {
        return null;
    };
...

{
    ...
    {
        var marker = new mxTerminalMarker(this.graph, mxConstants.CONNECT_TARGET_COLOR);
        var self = this;
        marker.getHighlightColor = function(state)
        {
            var src = (self.isSource) ? state.cell :
                      this.graph.getModel().getTerminal(self.state.cell, true);
            var trg = (self.isTarget) ? state.cell :
                      this.graph.getModel().getTerminal(self.state.cell, false);
            self.error = self.validateConnection(src, trg);
            if (self.error != null)
            {
                return mxConstants.INVALID_CONNECT_TARGET_COLOR;
            }
        ...
        var previous = marker.getHighlightCell;
        marker.getHighlightCell = function(evt, cell)
        {
            if (cell == null || !self.graph.model.isEdge(cell) || self.graph.connectableEdges)
            {
                return previous.apply(this, arguments);
    ...
    mxEdgeHandler.prototype.mouseDown = function(evt, cell, index)
    {
        ...
        {
            var source = index == 0;
            var target = index == this.bends.length - 1;
            this.isLabel = index == this.LABEL_INDEX;
            this.startX = evt.clientX;
            this.startY = evt.clientY;
            if (source || target)
            {
                var terminal = this.graph.model.getTerminal(cell, source);
                if (terminal == null || this.graph.isDisconnectable(cell, terminal, source))
                {
    ...
    mxEdgeHandler.prototype.mouseMove = function(evt, cell)
    {
        if (this.index != null && this.marker != null)
        {
            ...
            {
                if (this.graph.model.isConnectable(this.state.cell))
                {
                ...
                var p0 = this.abspoints[idx2];
                var pe = this.abspoints[idx];
                var dx = pe.x - p0.x;
                var dy = pe.y - p0.y;
                var len = Math.sqrt(dx * dx + dy * dy);
                point.x -= dx * 4 / len;
                point.y -= dy * 4 / len;
                this.abspoints[idx].x = this.graph.snap(point.x);
                this.abspoints[idx].y = this.graph.snap(point.y);
                ...
                var color = (this.error == null) ? mxConstants.VALID_COLOR : mxConstants.INVALID_COLOR;
                if (this.shape.dialect == mxConstants.DIALECT_SVG)
                {
                    this.shape.innerNode.setAttribute('stroke', color);
                }
            ...
            else
            {
                ...
                this.points[this.index - 1] = point;
    ...
    mxEdgeHandler.prototype.mouseUp = function(evt)
    {
        if (this.index != null && this.marker != null)
        {
            if (evt.clientX != this.startX || evt.clientY != this.startY)
            {
                ...
                if (this.isLabel)
                {
                    this.moveLabel(this.state.cell, this.label.x, this.label.y);
                    this.isLabel = false;
                }
                else if (this.isSource || this.isTarget)
                {
                    var term = model.getTerminal(this.state.cell, this.isSource);
                    if ((this.marker.cell != term || this.marker.cell == null) && this.error == null)
                    {
                        var edge = this.connect(this.state.cell, this.marker.cell, this.isSource, this.graph.isCloneable() && evt.ctrlKey);
                        if (edge != this.state.cell)
                        {
                            this.state = this.graph.getView().getState(edge);
                        }
                    }
                    else
                    {
                ...
                {
                    this.changePoints(this.state.cell, this.points);
                }
                this.abspoints = this.state.absolutePoints;
            }
            var p0 = this.abspoints[0];
            ...
            if (this.shape != null && this.shape.node != null)
            {
                var color = mxConstants.SELECTION_COLOR;
                if (this.shape.dialect == mxConstants.DIALECT_SVG)
                {
                    this.shape.innerNode.setAttribute('stroke', color);
                }
    ...
    {
        var model = this.graph.getModel();
        var geometry = model.getGeometry(edge).clone();
        var scale = this.graph.getView().scale;
        if (geometry.relative)
        {
            var pointCount = this.abspoints.length;
            var last = this.abspoints[1];
            var minDist = mxUtils.ptSegDistSq(this.abspoints[0].x, this.abspoints[0].y, last.x, last.y, x, y);
            var index = 0;
            var tmp = 0;
            var length = 0;
            for (var i = 2; i < pointCount; i++)
            {
                tmp += this.state.segments[i - 2];
                var dist = mxUtils.ptSegDistSq(this.abspoints[i].x, this.abspoints[i].y, last.x, last.y, this.label.x, this.label.y);
                if (dist <= minDist)
                {
                    minDist = dist;
                    index = i - 1;
                ...
                last = this.abspoints[i];
            }
            var seg = this.state.segments[index];
            var x2 = this.abspoints[index].x;
            var y2 = this.abspoints[index].y;
            var x1 = this.abspoints[index + 1].x;
            var y1 = this.abspoints[index + 1].y;
            var px = x;
            var py = y;
            x2 -= x1;
            y2 -= y1;
            ...
            var factor = tmp / seg;
            var p0 = this.abspoints[index];
            var pe = this.abspoints[index + 1];
            var offsetX = 0;
            var offsetY = 0;
            if (p0 != null && pe != null)
            {
                var dx = pe.x - p0.x;
                var dy = pe.y - p0.y;
                var tx = p0.x + dx * factor;
                var ty = p0.y + dy * factor;
                offsetX = (x - tx) / scale;
                offsetY = (y - ty) / scale;
            }
            geometry.y = 0;
            geometry.x = ((this.state.length / 2 - length - tmp) / this.state.length) * -2;
            geometry.offset = new mxPoint(offsetX, offsetY);
...

{
    function mxOutlineHandler(outline)
    {
        ...
        this.selectionBorder.dialect = (this.navigation.dialect != mxConstants.DIALECT_SVG) ? mxConstants.DIALECT_VML : mxConstants.DIALECT_SVG;
        this.selectionBorder.init(this.navigation.getView().getOverlayPane());
        var s = 3;
        ...
        this.sizer.init(this.navigation.getView().getOverlayPane());
    ...
    ;
    mxOutlineHandler.prototype.graph = null;
    mxOutlineHandler.prototype.navigation = null;
    mxOutlineHandler.prototype.update = function()
    {
        if (this.navigation.getView().currentRoot != this.graph.getView().currentRoot)
        {
            this.navigation.getView().setCurrentRoot(this.graph.getView().currentRoot);
        }
        var t = this.graph.view.translate;
        this.navigation.view.setTranslate(Math.max(0, t.x), Math.max(0, t.y));
        var t2 = this.navigation.getView().translate;
        var scale = this.graph.getView().scale;
        var scale2 = scale / this.navigation.getView().scale;
        var scale3 = 1.0 / this.navigation.getView().scale;
        var container = this.graph.container;
    ...
    mxOutlineHandler.prototype.mouseMove = function(evt, cell)
    {
        if (this.active)
        {
            ...
            if (this.index == null)
            {
                var scale = this.navigation.getView().scale;
    ...
    mxOutlineHandler.prototype.mouseUp = function(evt, cell)
    {
        if (this.active)
        {
            ...
            {
                this.graph.shift(0, 0);
                dx /= this.navigation.getView().scale;
                dy /= this.navigation.getView().scale;
                var t = this.graph.getView().translate;
                this.graph.getView().setTranslate(t.x - dx, t.y - dy);
            }
            else
            {
                ...
                var scale = this.graph.getView().scale;
                this.graph.getView().setScale(scale - (dx * scale) / w);
            }
            this.index = null;
            this.active = false;
            this.update();
...

{
    ...
    {
        if (!this.graph.panningHandler.isMenuShowing())
        {
...

{
    function mxHighlight(graph, color, funct)
    {
        this.graph = graph;
        this.graph.addGraphListener(this);
        this.marker = new mxTerminalMarker(graph, color, 0);
        this.marker.getHighlightCell = funct || function(evt, cell, isSource)
        {
            return cell;
        };
        if (mxClient.IS_IE)
    ...
    ;
    mxHighlight.prototype.graph = null;
    mxHighlight.prototype.marker = null;
    mxHighlight.prototype.enabled = true;
    mxHighlight.prototype.isEnabled = function()
    {
        return this.enabled;
    };
    mxHighlight.prototype.setEnabled = function(enabled)
    {
        this.enabled = enabled;
    };
...

{
        ...
        while (item != null)
        {
            if (item.nodeName == 'add')
            {
                ...
                if (condition == null || conditions[condition])
                {
                    ...
                    var row = this.addAction(menu, editor, as, icon, action, cell, parent);
    ...
    mxDefaultPopupMenu.prototype.addAction = function(menu, editor, lab, icon, action, cell, parent)
    {
    ...
    mxDefaultToolbar.prototype.connect = function(vertex, evt, source)
    {
        ...
        if (source != null && model.isConnectable(vertex) && graph.isEdgeValid(null, source, vertex))
        {
            ...
            try
            {
                ...
                var dist = source.getDirectedEdgeCount(true) * 20;
...

{
    ...
    mxEditor.prototype.selectSwimlane = true;
    ...
    mxEditor.prototype.addActions = function()
    {
        ...
        this.addAction('collapseAll', function(editor)
        {
            if (editor.graph.isEnabled())
            {
                var cells = editor.graph.getVertices(true);
        ...
        this.addAction('expandAll', function(editor)
        {
            if (editor.graph.isEnabled())
            {
                var cells = editor.graph.getVertices(false);
        ...
        this.addAction('zoom', function(editor)
        {
            ...
            var scale = parseFloat(prompt(mxResources.get(editor.askZoomResource) || editor.askZoomResource, current)) / 100;
            if (!isNaN(scale))
            {
    ...
    {
        var self = this;
        graph.addListener('click', function(sender, evt, cell)
        {
            if (cell == null && self.selectSwimlane && graph.isEnabled() && !mxEvent.isConsumed(evt))
            {

                var pt = mxUtils.convertPoint(graph.container, evt.clientX, evt.clientY);

                var swimlane = graph.getSwimlaneAt(pt.x, pt.y);
                if (swimlane != null)
                {
                    graph.selectCellForEvent(swimlane, evt);
                    mxEvent.consume(evt);
                }
            }
        });
    ...
    mxEditor.prototype.installUndoHandler = function(graph)
    {
        ...
        listener = function(sender, edit)
        {
            ...
            var tmp = graph.getModel().getTopmostCells(cells);
            ...
            if (self.maintainSwimlanes)
            {
                ...
                for (var i = 0; i < cellCount; i++)
                {
                    ...
                    if (graph.isSwimlane(cells[i]))
                    {
                        ...
                        if (parent != null && mxUtils.indexOf(visited, parent) < 0)
                        {
                            visited.push(parent);
    ...
    mxEditor.prototype.showOutline = function()
    {
        if (this.outline == null)
        {
            ...
            var wnd = new mxWindow(mxResources.get(this.outlineResource) || this.outlineResource, div, 600, 480, 200, null, false);

            var outline = new mxOutline(this.graph, div);
            var handler = new mxOutlineHandler(outline);
            wnd.setClosable(true);
            wnd.destroyOnClose = false;
            this.outline = wnd;
            this.outline.outline = outline;
            this.outline.handler = handler;
        }
        this.outline.setVisible(true);
        this.outline.handler.update();
    };
    ...
    mxEditor.prototype.addVertex = function(parent, vertex, x, y)
    {
        ...
        if (parent == null && this.swimlaneRequired)
        {
            return null;
        }
