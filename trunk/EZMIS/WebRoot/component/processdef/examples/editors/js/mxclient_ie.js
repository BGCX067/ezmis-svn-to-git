// You are licensed to use this software under the terms of the JGraph Evaluation License.// Do *not* use local copies of this code. Please ask for a copy if you require one.
var mxClient=
{
	VERSION:'1.0.0.1',
...
var imageNode=null;
if(imgName.substring(imgName.length-3,imgName.length).toUpperCase()=="PNG"&&true&&!true)
{
...
return(acp==bcp)?0:(((acp>bcp)==ascending)?1:-1);
});
return cells;
},
...
else if(roundColumnDownProportion>0&&roundColumnDownProportion<1!=null)
{
...
{
var node=null;
if(this.dialect==mxConstants.DIALECT_SVG&&(!false||!this.ENABLE_FOREIGNOBJECT))
{
node=this.createSvg();
}
...
if(imgName.substring(imgName.length-3,imgName.length)=="PNG"&&true&&!true)
{
...
if(bounds!=null)
{
var tmp=(horizontal)?bounds.x+bounds.width/2:
...
if(!graph.isHtmlLabel(state.cell)&&(value!=null&&!mxUtils.isNode(value))&&true)
{
...
var bounds=(state.view.graph.getModel().isEdge(state.cell))?new mxRectangle(state.x+state.width/2-w/2*s,state.y+state.height/2-h/2*s,w*s,h*s)
...
this.order(state);
if(state.text!=null&&state.text.node.parentNode==state.shape.node.parentNode)
{
state.shape.node.parentNode.insertBefore(state.text.node,state.shape.node.nextSibling);
}
reconfigure=true;
...
{
var box=(state.text!=null&&(!this.graph.isHtmlLabel(cell)||!this.graph.isLabelClipped(cell)))?state.text.boundingBox:null;
if(box!=null)
{
...
{
var edgeStyle=(source!=null&&source==target)?mxUtils.getValue(edgeState.style,mxConstants.STYLE_LOOP,this.graph.defaultLoopStyle):(!mxUtils.getValue(edgeState.style,mxConstants.STYLE_NOEDGESTYLE,false)?edgeState.style[mxConstants.STYLE_EDGE]:
null);
...
if(self.isContainerEvent(evt)&&((!true&&!false&&!false&&!false)||!self.isScrollEvent(evt)))
{
...
this.fireEvent(mxEvent.REMOVE_OVERLAY,new mxEventObject([cell,overlay]));
}
else
{
overlay=null;
...
for(var i=0;i<overlays.length;i++)
{
this.fireEvent(mxEvent.REMOVE_OVERLAY,new mxEventObject([cell,overlays[i]]));
}
}
return overlays;
};
...
if(cell!=null)
{
this.fireEvent(mxEvent.START_EDITING,new mxEventObject([cell,trigger]));
...
this.fireEvent(mxEvent.LABEL_CHANGED,new mxEventObject([cell,newValue,trigger]));
}
finally
{
...
this.fireEvent(mxEvent.ORDER_CELLS,new mxEventObject([back,cells]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.CELLS_ORDERED,new mxEventObject([cells,back]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.GROUP_CELLS,new mxEventObject([group,border,cells]));
}
finally
{
this.model.endUpdate();
...
bounds=new mxRectangle(x/scale-border-tr.x,y/scale-border-tr.y,width/scale+2*border,height/scale+2*border);
}
return bounds;
};
...
this.fireEvent(mxEvent.ADD_CELLS,new mxEventObject([cells,parent,index,source,target]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.CELLS_ADDED,new mxEventObject([cells,parent,index,source,target,absolute]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.REMOVE_CELLS,new mxEventObject([cells,includeEdges]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.SPLIT_EDGE,new mxEventObject([edge,cells,newEdge,dx,dy]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.TOGGLE_CELLS,new mxEventObject([show,cells,includeEdges]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.FOLD_CELLS,new mxEventObject([collapse,recurse,cells]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.CELLS_FOLDED,new mxEventObject([cells,collapse,recurse]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.UPDATE_CELL_SIZE,new mxEventObject([cell,ignoreChildren]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.RESIZE_CELLS,new mxEventObject([cells,boundsArray]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.CELLS_RESIZED,new mxEventObject([cells,boundsArray]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.MOVE_CELLS,new mxEventObject([cells,dx,dy,clone,target,evt]));
}
return cells;
};
...
this.fireEvent(mxEvent.CELLS_MOVED,new mxEventObject([cells,dx,dy,disconnect]));
}
finally
{
this.model.endUpdate();
...
{
var geo=this.model.getGeometry(cell);
var area=(this.isConstrainChild(cell))?this.getCellContainmentArea(cell):
...
this.fireEvent(mxEvent.CONNECT_CELL,new mxEventObject([edge,terminal,source]));
}
finally
{
this.model.endUpdate();
...
this.fireEvent(mxEvent.CELL_CONNECTED,new mxEventObject([edge,terminal,source]));
}
finally
{
this.model.endUpdate();
...
return(cell==null)?editingCell!=null:
cell==editingCell;
}
return false;
};
...
if(!true&&evtName==mxEvent.MOUSE_DOWN&&this.activeElement!=null)
{
...
{
if(!me.isConsumed()&&this.isEnabled()&&this.graph.isEnabled()&&!this.graph.isForceMarqueeEvent(me.getEvent())&&me.getHandle()==null&&me.getState()!=null)
{
var cell=me.getCell();
this.cell=null;
...
else(this.shape.dialect==mxConstants.DIALECT_VML)
{
var self=this;
...
{
self.icon=icon;
self.graph.fireMouseEvent(mxEvent.MOUSE_DOWN,new mxMouseEvent(evt,(self.currentState!=null)?self.currentState:state));
});
mxEvent.addListener(icon.node,'mousemove',function(evt)
{
self.graph.fireMouseEvent(mxEvent.MOUSE_MOVE,new mxMouseEvent(evt,(self.currentState!=null)?self.currentState:state));
});
mxEvent.addListener(icon.node,'mouseup',function(evt)
{
self.graph.fireMouseEvent(mxEvent.MOUSE_UP,new mxMouseEvent(evt,(self.currentState!=null)?self.currentState:state));
});
icons.push(icon);
...
{
if(!me.isConsumed()&&this.isEnabled()&&this.graph.isEnabled()&&!this.graph.isForceMarqueeEvent(me.getEvent())&&this.previous!=null&&this.error==null&&((this.icons==null)||(this.icons!=null&&this.icon!=null))&&me.getHandle()==null)
{
...
state.shape.node.style.cursor=(true)?'all-scroll':'default';
...
{
if(!me.isConsumed()&&this.isEnabled()&&this.graph.isEnabled()&&(this.graph.isForceMarqueeEvent(me.getEvent())||(me.getState()==null&&me.getHandle()==null)))
{
this.start(me.getX(),me.getY());
me.consume();
}
...
{
this.sizers.push(this.createSizer('default',mxEvent.LABEL_HANDLE,(this.graph.dialect==mxConstants.DIALECT_SVG)?4:6,mxConstants.LABEL_HANDLE_FILLCOLOR));
}
}
...
{
if(!me.isConsumed()&&this.graph.isEnabled()&&!this.graph.isForceMarqueeEvent(me.getEvent())&&this.state==me.getState()&&me.getHandle()!=null)
{
...
else if(this.graph.isEnabled()&&!this.graph.isForceMarqueeEvent(me.getEvent())&&this.marker!=null&&me.getHandle()!=null)
{
this.start(me.getX(),me.getY(),me.getHandle());
me.consume();
}
...
{
var edge=this.connect(edge,this.marker.getValidState().cell,this.isSource,this.graph.isCloneEvent(me.getEvent())&&this.cloneEnabled&&this.graph.isCellsCloneable());
}
...
if(evt!=null)
{
return(mxEvent.isControlDown(evt))?this.controlKeys[evt.keyCode]:
...
{
if(this.graph.isEditing()&&((evt.keyCode==13 &&!mxEvent.isControlDown(evt)&&!mxEvent.isShiftDown(evt))||(evt.keyCode==113 )))
{
this.enter(evt);
}
...
{
this.shape.dialect=(graph.dialect!=mxConstants.DIALECT_SVG)?mxConstants.DIALECT_VML:mxConstants.DIALECT_SVG;
...
this.shape.node.style.display='none';
this.edgeShape=new mxPolyline([new mxPoint(),new mxPoint()],highlightColor,strokeWidth);
...
{
var preview=new mxPrintPreview(editor.graph,1,null,0,0,null,'Printer-friendly version');
preview.open();
});
...
{
if(self.insertFunction!=null&&!me.isPopupTrigger()&&(self.forcedInserting||(me.getState()==null&&me.getHandle()==null)))
{
...
{
self.fireEvent(mxEvent.POST,new mxEventObject([req,url,data]));
});
};
...
this.fireEvent(mxEvent.BEFORE_ADD_VERTEX,new mxEventObject([vertex,parent]));
model.beginUpdate();
try
{
...
this.fireEvent(mxEvent.AFTER_ADD_VERTEX,new mxEventObject([vertex]));
}
return vertex;
};
...
if(isNode(obj))
{
node=(true)?obj.value.cloneNode(true):
