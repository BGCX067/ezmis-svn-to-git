// You are licensed to use this software under the terms of the JGraph
// Evaluation License.// Do *not* use local copies of this code. Please ask for
// a copy if you require one.
...
var mxUtils = {
	...
	show : function(graph, doc) {
		...
		if (mxClient.IS_IE) {
			...
			while (node != null) {
				var clone = (mxClient.IS_IE) ? node.cloneNode(true) : doc
							...
							if (graph.isEnabled() && !mxEvent.isConsumed(evt)) {
								...
								var dragHandler = function(evt) {
									...
									var pt = mxUtils.convertPoint(
											graph.container, evt.clientX,
											evt.clientY);
									sprite.style.left = (evt.clientX + origin.x + dx)
											+ 'px';
									sprite.style.top = (evt.clientY + origin.y + dy)
											+ 'px';
									if (!initialized) {
										initialized = true;
										...
										graph.scrollPointToVisible(pt.x, pt.y,
												graph.autoExtend);
									}
								...
								var dropHandler = function(evt) {
									mxEvent.removeListener(document,
...

{
	...
	mxWindow.prototype.show = function() {
		...
		if (!mxClient.IS_IE && (style.overflow == 'auto' || this.resize != null)) {
			this.contentWrapper.style.height = (this.div.offsetHeight - self.title.offsetHeight - 2) + 'px';
		}
		...
		mxEvent.addListener(img, 'mousedown', function(evt) {
			...
			if (factoryMethod != null) {
				...
				if (last != img) {
					...
					var point = new mxPoint(img.offsetLeft, img.offsetTop
							+ img.offsetHeight);
		...
		mxEvent
				.addListener(
						...
						function(evt) {
							...
							self.fireEvent(mxEvent.SELECT,
									new mxEventObject([null]));
							funct();
						});
		this.container.appendChild(img);
				...
				if (typeof(mxUtils) != 'undefined') {
					try {
						if (req.isReady() && req.getStatus() != 404) {
							...
							self.fireEvent(mxEvent.GET, new mxEventObject([url,
									req]));
		...
		if (!false && this.value != null && this.value.length > 0) {
			try {
				...
				this.boundingBox = new mxRectangle(Math.min(this.bounds.x,
						box.x - 4 * this.scale || 0), Math.min(this.bounds.y,
						box.y - 4 * this.scale || 0), Math.max(
						this.bounds.width, box.width + 8 * this.scale || 0),
						Math.max(this.bounds.height, box.height + 10
								* this.scale || 0));
	...
	mxLine.prototype.redrawSvg = function() {
		...
		if (this.bounds != null) {
			...
			this.pipe.setAttribute('stroke-width', this.strokewidth
					* this.scale + mxShape.prototype.SVG_STROKE_TOLERANCE);
			this.updateSvgTransform(this.innerNode, false);
		...
		if (node.cell != null && g != null) {
			...
			if (bounds == null) {
				...
				bounds = new mxRectangle(Math.min(bounds.x, g.x), Math.min(
						bounds.y, g.y), Math.max(bounds.x + bounds.width, g.x
						+ g.width), Math.max(bounds.y + bounds.height, g.y
						+ g.height));
			}
		}
		return bounds;
		...
		for (var i = 0;i < vertexCount; i++) {
			if (this.isVertexMovable(vertices[i])) {
				this.setVertexLocation(vertices[i], left + r + r
						* Math.sin(i * phi), top + r + r * Math.cos(i * phi));
			}
		}
	};
}
			...
			if (parent != null && edge != null) {
				...
				if (parentToCellRankDifference > 1) {
					...
					for (var i = edge.minRank + 1;i < edge.maxRank; i++) {
						...
						edge.setGeneralPurposeVariable(i, rankList[i].length
								- 1);
					}
				}
			}
...

{
	...
	mxPrintPreview.prototype.addPageClickListener = function(cell, pageNumber) {
		var self = this;
		mxEvent.addListener(cell, 'click',
				function(evt) {
					var page = self.wnd.document.getElementById('mxPage-'
							+ pageNumber);
					if (page != null) {
...

{
	...
	mxCellEditor.prototype.getEditorBounds = function(state) {
		...
		var result = new mxRectangle(state.x, state.y, Math.max(minWidth,
				state.width - spacingLeft - spacingRight), Math.max(minHeight,
				state.height - spacingTop - spacingBottom));
		if (isEdge) {
...

{
	...
	mxCellRenderer.prototype.createLabel = function(state) {
		...
		if (state.style[mxConstants.STYLE_FONTSIZE] > 0
			...
			mxEvent.addListener(state.text.node, 'dblclick', function(evt) {
				...
				//将dblclick编程showproperty
				editor.showProperties(state.cell);
	...
	mxCellRenderer.prototype.installCellOverlayListeners = function(state,
		...
		mxEvent.addListener(shape.node, 'click', function(evt) {
			overlay.fireEvent(mxEvent.CLICK,
					new mxEventObject([evt, state.cell]));
		});
	...
	mxCellRenderer.prototype.installListeners = function(state) {
		...
		mxEvent.addListener(state.shape.node, 'mousedown', function(evt) {
			...
			graph.fireMouseEvent(mxEvent.MOUSE_DOWN,
					new mxMouseEvent(evt, (state.shape != null && mxEvent
		...
		mxEvent.addListener(state.shape.node, 'dblclick', function(evt) {
			...
			graph
					.dblClick(evt, (state.shape != null && mxEvent
							.getSource(evt) == state.shape.content)
							? null
							: state.cell);
			mxEvent.consume(evt);
		...
		if (container != null) {
			...
			mxEvent.addListener(container, 'mousedown', function(evt) {
				...
				if (self.isContainerEvent(evt)
						&& ((!mxClient.IS_IE ) || !self.isScrollEvent(evt))) {
					graph.fireMouseEvent(mxEvent.MOUSE_DOWN,
...

{
	...
	mxGraph.prototype.getBoundsForGroup = function(group, children, border) {
		...
		if (children != null && children.length > 0 && bounds != null) {
			...
			bounds = new mxRectangle(x / scale - border - tr.x, y / scale
					- border - tr.y, width / scale + 2 * border, height / scale
					+ 2 * border);
		}
		return bounds;
	};
	...
	mxGraph.prototype.addCells = function(cells, parent, index, source, target) {
		...
		//判断是否是edge
		if(target != null && cells[0].value.tagName == "Edge"){
			//判断edge的target是否是开始流程
			if(target.value.tagName == "start-state") {
				alert("流程开始必须放在起始") ;
				return false ;
			}
			//判断edge的source是否是结束流程
			if(source.value.tagName == "end-state") {
				alert("结束流程必须放在最后") ;
				return false ;
			}
			//为edge赋予目标节点
			var name = getLabelValue("name",target) ;
			var edit = new mxCellAttributeChange(cells[0],"aimTask",name) ;
			editor.graph.model.execute(edit) ;
		}
		//判断是否是task
		if(cells[0].value.tagName == "task"){
			//写入流程属性中的url
			var taskCell = cells[0] ; 
			//同步process_url
			edit = new mxCellAttributeChange(taskCell,"process_url",getLabelValue("processPage",editor.graph.model.root)) ;
			editor.graph.model.execute(edit) ;
			/*
			//同步task中的process_url
			var jsonObj = Ext.decode(getLabelValue("task",taskCell).replace(/%26quot;/g,"'")) 
			jsonObj.process_url = getLabelValue("processPage",editor.graph.model) ;
	...
	mxGraph.prototype.validateGraph = function(cell, context) {
		...
		if (this.model.isEdge(cell)) {
			warning += this.getEdgeValidationError(cell, this.model
					.getTerminal(cell, true), this.model.getTerminal(cell,
					false))
					|| '';
		} else {
	...
	mxGraph.prototype.fireMouseEvent = function(evtName, me, source) {
		...
		if ((evtName != mxEvent.MOUSE_UP || this.isMouseDown)
			...
			if (!this.isEditing()
					&& (mxClient.IS_IE || mxClient.IS_OP || mxClient.IS_SF || me.getEvent().target != this.container)) {
...

{
	...
	mxOutline.prototype.update = function(revalidate) {
		...
		this.sizer.bounds = new mxRectangle(this.bounds.x + this.bounds.width
				- s, this.bounds.y + this.bounds.height - s, 2 * s, 2 * s);
		this.sizer.redraw();
		if (revalidate) {
	...
	mxPanningHandler.prototype.mouseDown = function(sender, me) {
		if (!me.isConsumed() && this.isEnabled()) {
			...
			if (this.popupTrigger && mxEvent.isRightMouseButton(me.getEvent())
					&& mxClient.IS_MAC && mxClient.IS_OP) {
	...
	mxConnectionHandler.prototype.createIcons = function(state) {
		...
		if (image != null && state != null) {
			...
			mxEvent.addListener(icon.node, 'mousedown', function(evt) {
				self.icon = icon;
				self.graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxMouseEvent(
						evt, (self.currentState != null)
								? self.currentState
								: state));
			});
			mxEvent.addListener(icon.node, 'mousemove', function(evt) {
				self.graph.fireMouseEvent(mxEvent.MOUSE_MOVE, new mxMouseEvent(
						evt, (self.currentState != null)
								? self.currentState
								: state));
			});
			mxEvent.addListener(icon.node, 'mouseup', function(evt) {
				self.graph.fireMouseEvent(mxEvent.MOUSE_UP, new mxMouseEvent(
						evt, (self.currentState != null)
								? self.currentState
								: state));
			});
			...
			if (this.previous != null && this.start != null) {
				...
				if (state != null) {
					...
					if (targetPerimeter != null) {
						...
						var tmp = targetPerimeter(view.getPerimeterBounds(
								state, null, false), null, state, false, next);
						if (tmp != null) {
							current = tmp;
						}
					} else {
				...
				if (this.selectedIcon != null) {
					...
					if (state != null && this.targetConnectImage) {
						...
						var bounds = new mxRectangle(pt.x, pt.y
								+ mxConstants.TOOLTIP_VERTICAL_OFFSET, w, h);
				...
				if (sourcePerimeter != null) {
					var tmp = sourcePerimeter(view.getPerimeterBounds(
							this.previous, null, true), null, this.previous,
							true, current);
					if (tmp != null) {
						pt = tmp;
					}
...

{
	...
	mxEdgeHandler.prototype.init = function() {
		...
		mxEvent.addListener(this.shape.node, 'dblclick', function(evt) {
			editor.showProperties(this.cell);
//			self.graph.dblClick(evt, self.state.cell);
//			mxEvent.consume(evt);
		});
		...
		mxEvent.addListener(this.shape.node, 'mousemove', function(evt) {
			...
			self.graph.fireMouseEvent(mxEvent.MOUSE_MOVE, new mxMouseEvent(evt,
					self.graph.getView().getState(cell)));
		});
		...
		mxEvent.addListener(this.labelShape.node, 'mousedown', function(evt) {
			self.graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxMouseEvent(evt,
					self.state, mxEvent.LABEL_HANDLE));
		});
		this.redraw();
	};
	...
	mxEdgeHandler.prototype.connect = function(edge, terminal, isSource,
		...
		try {
			...
			if (terminal == null) {
				...
				geo.setTerminalPoint(new mxPoint((current.x - dx) / scale
						- tr.x, (current.y - dy) / scale - tr.y), isSource);
				model.setGeometry(edge, geo);
...

{
	function mxCellHighlight(graph, highlightColor, strokeWidth) {
		if (graph != null) {
			...
			if (mxClient.IS_FF2 || mxClient.IS_FF3 || mxClient.IS_MAC) {
...

{
	...
	mxEditor.prototype.addActions = function() {
		...
		this.addAction('print', function(editor) {
			var preview = new mxPrintPreview(editor.graph, 1, null, 0, 0, null,
		...
		this.addAction('bold', function(editor) {
			if (editor.graph.isEnabled()) {
				editor.graph.toggleCellStyleFlags(mxConstants.STYLE_FONTSTYLE,
		...
		this.addAction('alignFontTop', function(editor) {
			if (editor.graph.isEnabled()) {
				editor.graph.setCellStyles(mxConstants.STYLE_VERTICAL_ALIGN,
							...
							function(sender) {
								...
								self
										.setStatus((mxResources
												.get(self.lastSavedResource) || self.lastSavedResource)
												+ ': ' + tstamp);
							});
			...
			this
					.addListener(
							...
							function(sender) {
								self
										.setStatus((mxResources
												.get(self.currentFileResource) || self.currentFileResource)
												+ ': ' + self.filename);
							});
			if (mxClient.IS_IE) {
				new mxDivResizer(container);
	...
	mxEditor.prototype.save_backcall = function(url,callback) {
		this.addListener(mxEvent.POST,callback);
		this.save(url);
	};
	...
	mxEditor.prototype.showProperties = function(cell) {
		...
		if (cell != null) {
			...
			var root = this.graph.getModel().getRoot();
			var argObject = {'mxModel':this.graph.getModel(),'mxCell':cell,'root':root};
			if(tagName == "Workflow") {
				showModule2("propWorkFlow/index.jsp",true,600,558,argObject);			
				return ;
			} else if(tagName == "task") {
				showModule2("propTaskNode/index.jsp",true,600,488,argObject);			
				return ;
			} else if(tagName == "Edge") {
				showModule2("propConnect/index.jsp",true,550,450,argObject);		
				return ;
			}else if(tagName == "fork"){
				showModule2("propJoin/index.jsp",true,320,240,argObject);
				return;
			}else if(tagName == "join"){
			
			}else if(tagName == "start-state"){
			
			}else if(tagName == "end-state"){
	...
	mxEditor.prototype.createProperties = function(cell) {
		...
		this.propertiesResource = value.nodeName + "Properties";
		if (mxUtils.isNode(value)) {
			...
			var getLabelValue= function(nodeName){
				for (var i = 0;i < attrs.length; i++) {
					if(attrs[i].nodeName == nodeName){
						return  attrs[i].nodeValue;
					}
			...
			for (var i = 0;i < attrs.length; i++) {
				...
				if(attrs[i].nodeName == 'label') continue;
				var val = "";
				if(attrs[i].nodeName == 'ä»»å¡åç§°'){
					if(getLabelValue('label') == 'ä»»å¡'){
						val = getLabelValue('label') +  cell.getId();
					}else{
						val = getLabelValue('label');
					}
				}else{
	...
	mxEditor.prototype.showTasks = function(tasks) {
		if (this.tasks == null) {
			...
			var wnd = new mxWindow(mxResources.get(this.tasksResource)
					|| this.tasksResource, div, w - 220, this.tasksTop, 200);
			wnd.setClosable(true);
	...
	mxEditor.prototype.showHelp = function(tasks) {
		if (this.help == null) {
			...
			var wnd = new mxWindow(mxResources.get(this.helpResource)
					|| this.helpResource, frame, (w - this.helpWidth) / 2,
					(h - this.helpHeight) / 3, this.helpWidth, this.helpHeight);
	...
	mxEditor.prototype.showOutline = function() {
		...
		if (create) {
			...
			var wnd = new mxWindow(mxResources.get(this.outlineResource)
					|| this.outlineResource, div, 600, 480, 200, 200, false);
	...
	mxCodec.prototype.encode = function(obj) {
		...
		if (obj != null && obj.constructor != null) {
			...
			if (enc != null) {
				...
				if (isNode(obj)) {
					node = (mxClient.IS_IE) ? obj.value.cloneNode(true) : this.document
...
mxCodecRegistry
		.register(function() {
			...
			codec.isExcluded = function(obj, attr, value, isWrite) {
				return mxObjectCodec.prototype.isExcluded
...
mxCodecRegistry.register(function() {
	var codec = new mxObjectCodec(new mxRootChange(), ['model', 'previous',
			'root']);
	codec.afterEncode = function(enc, obj, node) {
...
mxCodecRegistry.register(function() {
	var codec = new mxObjectCodec(new mxChildChange(), ['model', 'previous',
			'previousIndex', 'child'], ['parent']);
...
mxCodecRegistry.register(function() {
	return new mxObjectCodec(new mxGraph(), ['graphListeners',
			'eventListeners', 'view', 'container', 'cellRenderer', 'editor',
...
mxCodecRegistry
		.register(function() {
			...
			codec.encode = function(enc, view) {
				return this.encodeCell(enc, view, view.graph.getModel()
						.getRoot());
			};
			codec.encodeCell = function(enc, view, cell) {
				...
				for (var i = 0;i < childCount; i++) {
					node.appendChild(this.encodeCell(enc, view, model
							.getChildAt(cell, i)));
				}
				return node;
			};
						...
						if (node.nodeType == mxConstants.NODETYPE_ELEMENT) {
							if (!this.processInclude(dec, node, into)) {
								if (node.nodeName == 'separator') {
									...
									var pressedIcon = node
											.getAttribute('pressedIcon');
									var action = node.getAttribute('action');
									var mode = node.getAttribute('mode');
									var template = node
											.getAttribute('template');
									if (action != null) {
										into.addItem(as, icon, action,
												pressedIcon);
									} else if (mode != null) {
										var funct = mxUtils.eval(mxUtils
												.getTextContent(node));
										into.addMode(as, icon, mode,
												pressedIcon, funct);
										...
										into.addPrototype(as, icon, cell,
												pressedIcon, insertFunction);
									} else {
										var children = mxUtils
												.getChildNodes(node);
...
mxCodecRegistry.register(function() {
	var codec = new mxObjectCodec(new mxEditor(), ['modified', 'lastSnapshot',
			'ignoredChanges', 'undoManager', 'graphContainer',
			'toolbarContainer']);
	codec.afterDecode = function(dec, node, obj) {
		...
		while (tmp != null) {
			if (tmp.nodeName == 'add') {
				...
				if (elt != null) {
					...
					element.style.cssText = style;
					var wnd = new mxWindow(mxResources.get(as)=="工具栏"?"":mxResources.get(as) || as, element,
