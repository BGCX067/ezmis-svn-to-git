...
function WebFXColumnList() {
	...
	this.disabled		= false;												  //无效状态
	this.columnAlign    = true;                                                   // Enable column text alignment (true or false)
	this.sortAscImage   = 'SGJHTJ/component/columnlist/images/images/asc.png';                                       // Image used to indicate ascending sort order
	this.sortDescImage  = 'SGJHTJ/component/columnlist/images/images/desc.png';                                      // Image used to indicate descending sort order
	...
	this.columnWidthResizable=true; 
}
WebFXColumnList.prototype.disabled=true;
...
WebFXColumnList.prototype.selectRow = function(iRowIndex, bMultiple) {
	...
	if ((!bMultiple) || (!this.multiple)) {
		...
		while (this.selectedRows.length) {
			if (this.colorEvenRows) { eRows[this.selectedRows[0]].className = (this.selectedRows[0] & 1)?'odd':'even'; }
				...
				if(eRows[this.selectedRows[0]]) eRows[this.selectedRows[0]].className = ''; 
			}
	...
	/*
	 * 判断列中是否有输入框类型的列，如果有，在选中的时候让输入框为选中状态
	 * */

	var inputColIndex=-1;
	for(var i=0;i<this._aColumnTypes.length;i++){
		if(this._aColumnTypes[i]==TYPE_INPUT){
			inputColIndex=i;
			break;
		}
	}
	...
	if(inputColIndex>=0){
		var el = this._eBodyTable.tBodies[0].rows[iRowIndex].cells[inputColIndex];
...
WebFXColumnList.prototype.getSelectedRowObject = function() {
	var index=this.getSelectedRow();
	return this._eBodyTable.tBodies[0].rows[index];
}
WebFXColumnList.prototype.getSelectedRowValue = function(colIndex) {
	var iRow=this.getSelectedRow();
	var sValue="";
	
	if(parseInt(iRow)>=0){
		sValue=this.getCellValue(iRow,colIndex);
		//if(colIndex == 1)
		//alert(iRow+ ":" + colIndex+":" + sValue);
	}
...
WebFXColumnList.prototype.getCellValue = function(iRowIndex, iColIndex, bHTML) {
	...
	if(this._aColumnTypes[iColIndex]==TYPE_INPUT)
	...
	return (bHTML)?el.innerHTML:text;
};
...
WebFXColumnList.prototype._mouseDown = function(e) {
	...
	if ((el.tagName == 'TD') && (el.parentNode.parentNode.parentNode.parentNode.className == 'webfx-columnlist-head')) {
		...
		if (this._headerOper == COL_HEAD_EDGE) {
			if(this.columnWidthResizable==false)return;
			...
			if(this.columnMovable==false) return;
