...
Ext.ux.Multiselect = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
		...
		this.view = new Ext.ux.DDView({
			multiSelect: this.multiSelect,singleSelect:this.singleSelect, store: this.store, selectedClass: cls+"-selected", tpl:tpl,
	 ...
	 * 选择一个指定的item
	 */
	selectByIdx:function(idx){
		if(this.view){
			this.view.select(idx);
			var bt=new Ext.Panel();
	...
	appendItem:function(name,value){
		var MyRecord=new Ext.data.Record.create([{name:'name',mapping:'name'},{name:'value',mapping:'value'}]);
		/*var records=this.view.getSelectedRecords();
		var tmp=this;
		Ext.each(records,function(o){
			tmp.view.store.remove(o);
		})
		*/
		var record=new MyRecord({name:name,value:value});
		this.view.store.add(record);
		this.view.refresh();
		return this.view.store.indexOf(record);
	 ...
	 */
	size:function(){
		return this.view.store.getCount();
	},
