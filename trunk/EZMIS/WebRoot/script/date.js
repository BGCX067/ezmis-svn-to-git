...
function parseDate(val) {
	...
	generalFormats=new Array('y-M-d','MMM d, y','MMM d,y','y-MMM-d','d-MMM-y','MMM d', 'yyyy-MM-dd HH:mm:ss');
...
/**
 * 日期类型加减天数
 * @param {} val		日期对象 / 格式化为 yyyy-MM-dd 的日期字符串
 * @param {} dayCount	加减的天数(可以为负数)
 * @return {}			格式化为 yyyy-MM-dd 的日期字符串
 */
function getDataByDay(val,dayCount) {
	var newdate=new Date();
	if(typeof val == "string"){
		newdate=parseDate(val);
	}else{
		newdate=parseDate(formatDate(val,"yyyy-MM-dd"));
	}
	var newtimems=newdate.getTime()+(dayCount*24*60*60*1000);
 ...
 * 日期类型加减分钟
 * @param {} val		日期对象 / 格式化为 yyyy-MM-dd 的日期字符串
 * @param {} dayCount	加减的分钟数(可以为负数)
 * @return {}			格式化为 yyyy-MM-dd 的日期字符串
 */
function getDataByMine(val,dayCount) {
	var newdate=new Date();
	if(typeof val == "string"){
		newdate=parseDate(val);
	}else{
	...
	var newtimems=newdate.getTime()+(dayCount*60*60);
	newdate.setTime(newtimems);
	return formatDate(newdate,"yyyy-MM-dd");
}
 ...
 * @return {}			格式化为 yyyy-MM-dd 的日期字符串
 */
function getDataByDayWithFormat(val,dayCount, format) {
	var newdate=new Date();
	if(typeof val == "string"){
		newdate=parseDate(val);
	}else{
		newdate=parseDate(formatDate(val,format));
	}
	var newtimems=newdate.getTime()+(dayCount*24*60*60*1000);
	newdate.setTime(newtimems);
