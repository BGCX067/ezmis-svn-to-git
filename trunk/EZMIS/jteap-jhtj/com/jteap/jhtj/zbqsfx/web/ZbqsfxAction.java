package com.jteap.jhtj.zbqsfx.web;
...
import com.jteap.jhtj.zbqsfx.manager.ZbqsfxManager;
@SuppressWarnings({ "unchecked", "serial" })
public class ZbqsfxAction extends AbstractTreeAction {
	private TjItemKindManager tjItemKindManager;
	private ZbqsfxManager zbqsfxManager;
	...
	public String generateChartAction() throws Exception{
		...
		//判断选择的日期是否有效
		boolean result=this.zbqsfxManager.isDateLawful(keyList, flflag);
		if(result){
			MSLineChart chart=this.zbqsfxManager.generateChart(keyList, item, kid, flflag);
			outputJson("{success:true,chartData:'"+chart.toString()+"',tableData:["+this.zbqsfxManager.getTableToJson(chart)+"]}");
			//System.out.println("{success:true,chartData:'"+chart.toString()+"',tableData:["+chart.getTableToJson()+"]}");
		}else{
	...
	public ZbqsfxManager getZbqsfxManager() {
		return zbqsfxManager;
	}

	public void setZbqsfxManager(ZbqsfxManager zbqsfxManager) {
		this.zbqsfxManager = zbqsfxManager;
	}
