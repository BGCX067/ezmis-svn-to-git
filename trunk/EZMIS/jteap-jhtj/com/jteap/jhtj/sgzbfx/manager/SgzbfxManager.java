package com.jteap.jhtj.sgzbfx.manager;
...
public class SgzbfxManager extends HibernateEntityDao<TjItemKind> {
	/**
	 * 
	 *描述：判断日期是否合法
	 *时间：2010-5-6
	...
	public MSColumn3DChart generateChart(String keyList,String fields,String kid,String flflag) throws Exception{
		//图形
		...
		try{
			...
			for(int i=0;i<items.length;i++){
				String[] itemAndCname=items[i].split(",");
				
				MSColumn3DDataSet dataSet=new MSColumn3DDataSet();
				dataSet.setSeriesName(itemAndCname[1]);		
				...
				//日数据
				if("1".equals(flflag)){
					field.append(TjInterFace.NIAN+","+TjInterFace.YUE+","+TjInterFace.RI+","+itemAndCname[0]);
				...
				while(rs.next()){
					...
					for(int j=1;j<=rsmeta.getColumnCount();j++){
						...
						map.put(rsmeta.getColumnName(j).toString(), (obj==null?"":obj.toString()));
					}
					rowRecords.add(map);
				}
				
				//初始化起始时间到终止时间之间的每天的值
				List<Map<String,String>> allData=this.initQssjToZzsjData(keyMap, flflag, itemAndCname[0]);
				...
				for(Map<String,String> itemMap:allData){
					...
					for(Map<String,String> recordMap:rowRecords){
						...
						if("1".equals(flflag)){
							if(nian.equals(reNian)&&yue.equals(reYue)&&ri.equals(reRi)){
								itemMap.put(itemAndCname[0], recordMap.get(itemAndCname[0]));
							}
						}else if("2".equals(flflag)){
					...
					set.setValue(itemMap.get(itemAndCname[0]));
