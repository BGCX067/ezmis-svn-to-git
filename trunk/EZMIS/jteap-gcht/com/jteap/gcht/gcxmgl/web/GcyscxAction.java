...
import com.jteap.gcht.gcxmgl.manager.GcyscxManager;
import com.jteap.gcht.gcxmgl.model.Kgqz;
import com.jteap.gcht.gcxmgl.model.Ysd;

/**
 * @author 王艺
 * @version 创建时间：Jun 15, 2012 1:53:19 PM
 * 类说明
 */

public class GcyscxAction extends AbstractAction{

	private GcyscxManager gcyscxManager;
	
	
	public GcyscxManager getGcyscxManager() {
		return gcyscxManager;
	}

	public void setGcyscxManager(GcyscxManager gcyscxManager) {
		this.gcyscxManager = gcyscxManager;
	}
	...
	public String showListAction() throws Exception {
		...
		String sql = "";
		try {
			sql = "select * from tb_ht_ysd ysd where ysd.id != ' ' ";

			if (StringUtils.isNotEmpty(xmbh)) {
				sql += " and (ysd.gcxmbh like '%" + xmbh + "%')";
			}
			if (StringUtils.isNotEmpty(status)) {
				if(status.equals("1")){
					sql += " and (ysd.status = '已终结')";
				}else{
					sql += " and (ysd.status != '已终结')";
				}
			}
			if (StringUtils.isNotEmpty(sqbmqz)) {
				sql += " and (ysd.sqr = '%" + sqbmqz + "%')";
			}
			...
			String json = JSONUtil.listToJson((List) page.getResult(),
					new String[] {"ID", "GCXMBH", "GCXMMC", "SGDW", "KGSJ","JGSJ","SQBM","SQR","XMYJ","CJSJ","STATUS","ISZF"});
...
//		double jesum = 0.0;
		...
		try {
			for (int i = 0; i < idsArray.length; i++) {
				Ysd ysd = gcyscxManager.get(idsArray[i]);
				if(i == 0 || flag == true){
					// 创建行
					if(i == 0){
				...
				cell.setCellValue(ysd.getGcxmbh());
				cell = row.createCell(1);
				cell.setCellStyle(style2);
				cell.setCellValue(ysd.getStatus());
				cell = row.createCell(2);
				cell.setCellStyle(style2);
				cell.setCellValue(ysd.getSqbm());
				cell = row.createCell(3);
				cell.setCellStyle(style2);
				cell.setCellValue(ysd.getSqr());
				cell = row.createCell(4);
				cell.setCellStyle(style2);
				SimpleDateFormat format = new SimpleDateFormat(
				"yyyy-MM-dd HH:mm:ss");
				cell.setCellValue(format.format(ysd.getCjsj()));
				cell = row.createCell(5);
				cell.setCellStyle(style2);
				cell.setCellValue(ysd.getXmyj());
				cell = row.createCell(6);
				cell.setCellStyle(style2);
				cell.setCellValue(ysd.getGcxmmc());
				cell = row.createCell(7);
				cell.setCellStyle(style2);
				cell.setCellValue(ysd.getIszf());
				count++;
