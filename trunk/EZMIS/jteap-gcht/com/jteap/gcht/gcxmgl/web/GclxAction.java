...
import com.jteap.gcht.gcxmgl.manager.GclxManager;
...
public class GclxAction extends AbstractAction{
	
	private GclxManager gclxManager;
	...
	public void setGclxManager(GclxManager gclxManager) {
		this.gclxManager = gclxManager;
	}
	...
	public String showDspHeTongAction() throws Exception {
		...
		try {
			...
			Page page = gclxManager.pagedQueryTableData(sql, iStart, iLimit);
			...
			String json = JSONUtil.listToJson((List) page.getResult(),new String[]{
					...
					"ID", "XMBH", "XMMC", "XMLX","XMYJ","CBFS","GCYS","KGRQ","SQBM","CJSJ","STATUS"
			});
...
//			sbSql.append("and a.status != '已终结' ");
			...
			sbSql.append("(select 1 from tb_wf_todo f  where f.docid = a.id and f.flow_instance_id = c.id_ and f.flag = '1' and f.CURRENT_PROCESS_PERSON like '%"+userLoginName+"%')");
...
//			}
	...
	public String cancelHeTongAction() throws Exception {
		...
		String tableName = request.getParameter("tableName");
		String zfyy = request.getParameter("");
	...
	public String findCurrentXmxhAction(){
		try {
			...
			int xmxh = gclxManager.findCurrentXmxh(tableName,zy);
			this.outputJson("{success:true,xmxh:'"+xmxh+"'}");
		}catch (Exception e) {
			e.printStackTrace();
		}
		return NONE;
