...
import com.jteap.gcht.gcxmgl.manager.JszfcxManager;
import com.jteap.gcht.gcxmgl.model.Gcxmjszfd;
import com.jteap.gcht.gcxmgl.model.Ysd;

/**
 * @author 王艺
 * @version 创建时间：Jun 15, 2012 1:54:44 PM
 * 类说明
 */

public class JszfcxAction extends AbstractAction{
	
	private JszfcxManager jszfcxManager;

	
	public JszfcxManager getJszfcxManager() {
		return jszfcxManager;
	}

	public void setJszfcxManager(JszfcxManager jszfcxManager) {
		this.jszfcxManager = jszfcxManager;
	}
	...
	public String showListAction() throws Exception {
		...
		String sql = "";
		try {
			sql = "select * from tb_ht_gcxmjszfd zfd where zfd.id != ' ' ";

			if (StringUtils.isNotEmpty(xmbh)) {
				sql += " and (zfd.xmbh like '%" + xmbh + "%')";
			}
			if (StringUtils.isNotEmpty(status)) {
				if(status.equals("1")){
					sql += " and (zfd.status = '已终结')";
				}else{
					sql += " and (zfd.status != '已终结')";
				}
			}
			if (StringUtils.isNotEmpty(sqbmqz)) {
				sql += " and (zfd.sqr = '%" + sqbmqz + "%')";
			}
			...
			String json = JSONUtil.listToJson((List) page.getResult(),
					new String[] { "ID", "XMBH","XMMC", "CBDW","SQBM","SQR","SQSJ",
					"XMHTJE", "YLJYFKJE", "BCYFK", "BCSFK", "BCKK", "CJSJ","XMFYLY",
					"FQZT","ZFZT","STATUS"});
...
//		double jesum = 0.0;
		...
		try {
			for (int i = 0; i < idsArray.length; i++) {
				Gcxmjszfd zfd = jszfcxManager.get(idsArray[i]);
				if(i == 0 || flag == true){
					// 创建行
					if(i == 0){
				...
				cell.setCellValue(zfd.getXmbh());
				cell = row.createCell(1);
				cell.setCellStyle(style2);
				cell.setCellValue(zfd.getStatus());
				cell = row.createCell(2);
				cell.setCellStyle(style2);
				cell.setCellValue(zfd.getSqbm());
				cell = row.createCell(3);
				cell.setCellStyle(style2);
				cell.setCellValue(zfd.getSqr());
				cell = row.createCell(4);
				cell.setCellStyle(style2);
				SimpleDateFormat format = new SimpleDateFormat(
				"yyyy-MM-dd HH:mm:ss");
				cell.setCellValue(format.format(zfd.getSqsj()));
				cell = row.createCell(5);
				cell.setCellStyle(style2);
				cell.setCellValue(zfd.getXmyj());
				cell = row.createCell(6);
				cell.setCellStyle(style2);
				cell.setCellValue(zfd.getXmmc());
				cell = row.createCell(7);
				cell.setCellStyle(style2);
				cell.setCellValue(zfd.getXmhtje());
				count++;
