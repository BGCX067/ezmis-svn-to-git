...
import com.jteap.gcht.gcxmgl.manager.KgqzcxManager;
import com.jteap.gcht.gcxmgl.model.Kgqz;
import com.jteap.gcht.gcxmgl.model.Wtd;

/**
 * @author 王艺
 * @version 创建时间：Jun 15, 2012 9:03:03 AM
 * 类说明
 */

public class KgqzcxAction extends AbstractAction{
	private KgqzcxManager kgqzcxManager;
	
	
	public KgqzcxManager getKgqzcxManager() {
		return kgqzcxManager;
	}

	public void setKgqzcxManager(KgqzcxManager kgqzcxManager) {
		this.kgqzcxManager = kgqzcxManager;
	}
	...
	public String showListAction() throws Exception {
		...
		String sql = "";
		try {
			sql = "select * from tb_ht_kgqz kgd where kgd.id != ' ' ";

			if (StringUtils.isNotEmpty(xmbh)) {
				sql += " and (kgd.gcxmbh like '%" + xmbh + "%')";
			}
			if (StringUtils.isNotEmpty(status)) {
				if(status.equals("1")){
					sql += " and (kgd.status = '已终结')";
				}else{
					sql += " and (kgd.status != '已终结')";
				}
			}
			if (StringUtils.isNotEmpty(sqbmqz)) {
				sql += " and (kgd.xmfzr = '%" + sqbmqz + "%')";
			}
			...
			String json = JSONUtil.listToJson((List) page.getResult(),
					new String[] { "ID", "GCXMBH", "GCXMMC", "GCCBDW", "JHKGRQ","JHGQ","SQBM","XMFZR","XMYJ","CJSJ","STATUS","ISYS"});
	...
	/**
	 * 
	 * 描述 : 获取具体一个流程实例 作者 : wangyi 时间 : Oct 28, 2012 参数 : 返回值 : 异常 :
	 */
	public String showProcessinstance() throws Exception {
		String kgid = request.getParameter("kgid");
		Connection conn = null;
		DataSource dataSource = (DataSource) SpringContextUtil
				.getBean("dataSource");
		try {
...
//		double jesum = 0.0;
		...
		try {
			for (int i = 0; i < idsArray.length; i++) {
				Kgqz kgqz = kgqzcxManager.get(idsArray[i]);
				if(i == 0 || flag == true){
					// 创建行
					if(i == 0){
				...
				cell.setCellValue(kgqz.getGcxmbh());
				cell = row.createCell(1);
				cell.setCellStyle(style2);
				cell.setCellValue(kgqz.getStatus());
				cell = row.createCell(2);
				cell.setCellStyle(style2);
				cell.setCellValue(kgqz.getSqbm());
				cell = row.createCell(3);
				cell.setCellStyle(style2);
				cell.setCellValue(kgqz.getXmfzr());
				cell = row.createCell(4);
				cell.setCellStyle(style2);
				SimpleDateFormat format = new SimpleDateFormat(
				"yyyy-MM-dd HH:mm:ss");
				cell.setCellValue(format.format(kgqz.getCjsj()));
				cell = row.createCell(5);
				cell.setCellStyle(style2);
				cell.setCellValue(kgqz.getXmyj());
				cell = row.createCell(6);
				cell.setCellStyle(style2);
				cell.setCellValue(kgqz.getGcxmmc());
				cell = row.createCell(7);
				cell.setCellStyle(style2);
				cell.setCellValue(kgqz.getIsys());
				count++;
	...
	private TIMESTAMP timestamp(Date cjsj) {
		// TODO Auto-generated method stub
		return null;
	}
