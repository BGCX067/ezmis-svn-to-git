 ...
 *
 */
public class RlHeTongNodeInitEventHandler extends InitNodeEventHandler {
	...
	public void execute(ExecutionContext context) throws Exception {
		...
		try {
			...
			} else if ("燃料供应部副主任审批".equals(taskNode.getName())) {
				status = "申请审批";
				context.getContextInstance().setVariable("STATUS", status);
			} else if ("燃料供应部主任审批".equals(taskNode.getName())) {
				status = "燃料供应部副主任已审批";
				context.getContextInstance().setVariable("STATUS", status);
